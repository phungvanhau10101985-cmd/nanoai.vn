...
| ID | Hạng mục | Trạng thái | Ghi chú / file neo |
|----|----------|------------|-------------------|
| W2.1 | Visual editor (text/ảnh/section bg) | ✅ Done | `partner-website-visual-editor-toolbar.tsx`. **🚫 Chat AI tạo/chỉnh web** gỡ 2026-08-13. **2026-08-14**: Sửa nhanh mọi trang + **tách bản Mobile/Desktop** (`*.html` / `*.mobile.html`). **2026-08-16**: thêm bản **Tablet** (`*.tablet.html`) — 3 giao diện độc lập (Mobile / máy tính bảng / Desktop; Laptop xem chung bản máy tính). **2026-08-19**: thống nhất Sửa nhanh / dashboard preview / `/site/{slug}` / custom domain qua `render-partner-visual-html.ts`, `?pw-device` trả đúng file isolated của máy. PDP không serve HTML đóng băng. |
...
| W4.9 | **Trang danh mục**: banner, thẻ danh mục con, breadcrumb, lưới sản phẩm, phân trang | ✅ Done (2026-08-05; UX 188 cập nhật 2026-08-26) | W4.7 | Breadcrumb + banner + tile con + lưới. **W4.14:** gộp SP nhánh con, phân trang số, filter/sort trên URL |
...
| W4.14 | **UX danh mục khớp 188** — mega menu 2 cột, hub `/c`, gộp nhánh, filter sticky + sort 188, phân trang số, canonical filter | ✅ Done (2026-08-26) | W4.8, W4.9, W4.11 | Rule `.cursor/rules/partner-website-categories.mdc`. Không copy cluster/dual-taxonomy/random mặc định |
...
| 2026-08-26 | **W4.14** | UX danh mục khớp 188: mega menu 2 cột L1\|L2/L3 (HTML + React), hub `/c`, gộp SP nhánh con, filter sticky + sort (Ngẫu nhiên/Mới nhất/Cũ nhất/Xem nhiều) trên URL, phân trang số, canonical whitelist, ẩn hậu duệ khi tắt cha. Rule `partner-website-categories.mdc`. Không copy cluster/dual-taxonomy/random mặc định. | ✅ Done |
