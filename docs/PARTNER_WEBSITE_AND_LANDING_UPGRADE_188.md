...
| W2.3 | Merchant edit nav / page tree / footer / theme tokens | 🚫 Removed (2026-08-13) | Gỡ panel màu theme + menu/footer + PATCH `update_theme_colors` / `update_nav_footer`. Shop shell vẫn đọc `theme_json`/`nav_json`/`footer_json` mặc định. Trang chủ chỉnh bằng Sửa nhanh (HTML). |
