...
import { downloadStandaloneBagMockupHtml } from '@/lib/packaging/mockup-share-bag-html'
import {
  bagMockupDownloadFilename,
...
> = {
  vi: {
    share: 'Chia sẻ',
    dialogTitle: 'Chia sẻ mockup 3D túi',
    dialogDescription: 'Gửi link hoặc file HTML — người nhận xoay túi 3D thật trên trình duyệt.',
    ...
    htmlNote: 'File HTML: mở bằng Chrome/Edge/Safari. Cần mạng để tải ảnh mặt trước/sau.',
  },
  en: {
    share: 'Share',
    dialogTitle: 'Share 3D bag mockup',
    dialogDescription: 'Send a link or HTML file — recipients rotate the real 3D bag in their browser.',
    ...
    htmlNote: 'HTML file: open in Chrome/Edge/Safari. Internet required to load front/back images.',
  },
  zh: {
    share: '分享',
    dialogTitle: '分享 3D 纸袋 mockup',
    dialogDescription: '发送链接或 HTML 文件 — 对方可在浏览器中旋转真实 3D 纸袋。',
    copyLink: '复制交互链接（推荐）',
    ...
    htmlNote: 'HTML 文件：用 Chrome/Edge/Safari 打开。需联网加载正/背面图片。',
  },
  ja: {
    share: '共有',
    dialogTitle: '3D紙袋モックアップを共有',
    dialogDescription: 'リンクまたはHTMLファイルを送る — 受け取った人がブラウザで3D紙袋を回転できます。',
    ...
    htmlNote: 'HTML：Chrome/Edge/Safariで開く。表/裏画像の読み込みにネット接続が必要です。',
  },
  ko: {
    share: '공유',
    dialogTitle: '3D 종이봉투 목업 공유',
    dialogDescription: '링크 또는 HTML 파일 전송 — 받는 사람이 브라우저에서 3D 종이봉투를 회전할 수 있습니다.',
    ...
    htmlNote: 'HTML: Chrome/Edge/Safari에서 열기. 앞/뒷면 이미지 로드에 인터넷 필요.',
  },
}
...
export function PackagingBagMockupShareMenu({
  dimensionsMm,
  faceSlots,
  locale,
  overlayClassName,
  ...
  const baseFilename = bagMockupDownloadFilename(dimensionsMm)
  ...
  const onDownloadHtml = () => {
    ...
    try {
      ...
      downloadStandaloneBagMockupHtml({
        dimensionsMm,
        faceUrls,
