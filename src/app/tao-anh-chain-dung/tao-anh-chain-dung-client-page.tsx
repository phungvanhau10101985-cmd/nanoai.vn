...
import { Upload, Sparkles, Download, RefreshCw, Link2, Briefcase } from 'lucide-react'
...
export default function TaoAnhChainDungClientPage() {
  ...
  const handleSubmit = async () => {
    if (!image.file) {
      toast({ title: 'Lỗi', description: 'Vui lòng tải lên ảnh selfie.', variant: 'destructive' })
      return
    }
    setStep('GENERATING')
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground flex items-center justify-center gap-2">
            <Briefcase className="h-8 w-8 text-slate-600" /> Tạo ảnh chân dung chuyên nghiệp
          </h1>
          <p className="text-muted-foreground mt-1">Biến selfie thành ảnh LinkedIn, CV. Áo vest, ánh sáng studio. 2–4 credits/ảnh.</p>
        </div>
              ...
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-slate-200/60">
                <CardHeader className="p-4 pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Upload className="h-4 w-4 text-slate-600" /> Ảnh selfie
                  </CardTitle>
                  <CardDescription className="text-xs">Chọn ảnh selfie bình thường. AI sẽ chỉnh thành ảnh chuyên nghiệp.</CardDescription>
                </CardHeader>
                <CardContent className="p-4 pt-0 space-y-4">
                  ...
                  >
                    {image.preview ? (
                      ...
                      <>
                        ...
                        <p className="text-sm text-muted-foreground font-medium">Chọn ảnh selfie</p>
                      </>
                    )}
                  </label>
                  {image.preview && (
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea placeholder="VD: áo vest xanh, nền tối..." value={note} onChange={(e) => setNote(e.target.value)} className="bg-white/80 text-xs h-16 resize-y" />
                  </div>
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="pt-4 border-t space-y-2 flex flex-col items-center">
                    ...
                    <Button onClick={() => checkCreditsAndProceed(cost, handleSubmit)} disabled={!image.file} className="w-full max-w-[180px] h-9 shadow-md hover:shadow-lg transition-all text-sm bg-slate-600 hover:bg-slate-700 text-white">
                      <Sparkles className="mr-2 h-4 w-4" /> Tạo ảnh ({imageQuality === '2K' ? '2' : '4'} credit)
                    </Button>
        ...
        {step === 'GENERATING' && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur border-slate-200/60">
            <CardContent className="flex flex-col items-center py-8">
              <ImageProcessingLoader mode="headshot" title="Đang tạo ảnh chân dung" description="AI đang chỉnh trang phục, ánh sáng studio" imagePreview={image.preview} />
            </CardContent>
          </Card>
        )}
        ...
        {step === 'RESULT' && resultUrl && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription>Đã tạo ảnh chân dung chuyên nghiệp.</CardDescription>
            </CardHeader>
