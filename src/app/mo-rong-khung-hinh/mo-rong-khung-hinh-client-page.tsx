...
import { Upload, Sparkles, Download, RefreshCw, Link2, Expand } from 'lucide-react'
...
export default function MoRongKhungHinhClientPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (result.error) {
      ...
      setStep('RESULT')
      toast({ title: 'Thành công!', description: 'Đã mở rộng khung hình.', duration: 3000 })
    }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground flex items-center justify-center gap-2">
            <Expand className="h-8 w-8 text-indigo-600" /> Mở rộng khung hình
          </h1>
          <p className="text-muted-foreground mt-1">Ảnh dọc thành banner ngang. AI vẽ thêm nền tự nhiên. 1,5–3 credits/ảnh.</p>
        </div>
              ...
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-indigo-200/60">
                <CardHeader className="p-4 pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Upload className="h-4 w-4 text-indigo-600" /> Ảnh cần mở rộng
                  </CardTitle>
                  <CardDescription className="text-xs">Ảnh chụp dọc muốn thành banner ngang. AI vẽ thêm nền hai bên.</CardDescription>
                </CardHeader>
            ...
            <div className="lg:w-[200px] lg:shrink-0">
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-indigo-200/60 h-full">
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardDescription className="text-xs">Tỷ lệ và chất lượng.</CardDescription>
                </CardHeader>
                <CardContent className="p-4 space-y-4">
                  <div className="space-y-2">
                    <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Tỷ lệ khung hình</h4>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input placeholder="VD: nền biển, nền studio..." value={note} onChange={(e) => setNote(e.target.value)} className="bg-white/80 text-xs h-9" />
                  </div>
                  <div className="pt-4 border-t space-y-2 flex flex-col items-center">
                    ...
                    <Button onClick={() => checkCreditsAndProceed(cost, handleSubmit)} disabled={!image.file} className="w-full max-w-[180px] h-9 shadow-md hover:shadow-lg transition-all text-sm bg-indigo-600 hover:bg-indigo-700 text-white">
                      <Sparkles className="mr-2 h-4 w-4" /> Mở rộng ({imageQuality === '2K' ? '1,5' : '3'} credit)
                    </Button>
        ...
        {step === 'GENERATING' && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur border-indigo-200/60">
            <CardContent className="flex flex-col items-center py-8">
              <ImageProcessingLoader mode="outpaint" title="Đang mở rộng khung hình" description="AI đang vẽ thêm nền tự nhiên" imagePreview={image.preview} />
            </CardContent>
          </Card>
        )}
        ...
        {step === 'RESULT' && resultUrl && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription>Đã mở rộng khung hình.</CardDescription>
            </CardHeader>
