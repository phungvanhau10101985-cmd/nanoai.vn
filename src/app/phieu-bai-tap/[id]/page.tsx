...
export default async function PhieuBaiTapPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-gradient-to-b from-emerald-50/90 via-background to-background dark:from-emerald-950/25 dark:via-background dark:to-background">
      <div
      ...
      <div className="relative mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-10">
        <nav className="mb-8 flex flex-wrap items-center justify-between gap-3">
          ...
          <Link
            href="/"
            className="text-xs font-medium text-emerald-700/90 underline-offset-4 hover:underline dark:text-emerald-400/90"
          >
            {ws.backHome}
          </Link>
        </nav>
        ...
        <WorksheetSharePanel
          ...
          labels={{
            title: lt('Chia sẻ & tải phiếu bài tập', 'Share & download worksheet', '分享与下载练习', 'ワークシート共有・ダウンロード', '워크시트 공유/다운로드'),
            hint: ws.qrHint,
            copy: lt('Sao chép', 'Copy', '复制', 'コピー', '복사'),
            copied: lt('Đã sao chép nội dung phiếu bài tập.', 'Worksheet content copied.', '练习内容已复制。', 'ワークシート内容をコピーしました。', '워크시트 내용을 복사했습니다.'),
