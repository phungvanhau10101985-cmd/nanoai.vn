    ...
    try {
      ...
      await finalizeStandardImageGenerationResult(result, {
        onServerErrorMessage: (message) => {
          setStep('UPLOAD')
          toast({
            title: tr('Ghép ảnh thất bại', 'Image merge failed', '合成失败', '画像合成に失敗しました', '이미지 합성 실패'),
            description: message,
            variant: 'destructive',
            duration: 5000,
          })
        ...
        onSuccessWithUrl: (url) => {
          ...
          toast({
            ...
            description: tr('Ảnh đã được ghép.', 'Images have been merged.', '图片已合成。', '画像を合成しました。', '이미지 합성이 완료되었습니다.'),
            duration: 3000,
          })
        },
        onUnexpectedPayload: () => {
