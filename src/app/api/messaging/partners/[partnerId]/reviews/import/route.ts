...
import { parseReviewImportWorkbook } from '@/lib/partner-website/reviews/partner-reviews-qa-excel'
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ partnerId: string }> }) {
  ...
  const drafts = parseReviewImportWorkbook(buffer)
  if (drafts.length === 0) {
    return NextResponse.json({ error: 'empty_file', created: 0 }, { status: 400 })
  }
  const created = await insertImportedPartnerProductReviewsFromPg(pid, drafts)
