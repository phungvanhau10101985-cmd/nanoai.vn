...
function resolvePublishedSiteSlug(partnerId: string) {
  return fetchPartnerWebsiteByPartnerIdPg(partnerId).then((site) => {
    if (!site?.isPublished) return null
    return site.siteSlug.trim() || null
  })
}
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ partnerId: string }> }) {
  ...
  const siteSlug = await resolvePublishedSiteSlug(partnerId)
  if (!siteSlug) {
