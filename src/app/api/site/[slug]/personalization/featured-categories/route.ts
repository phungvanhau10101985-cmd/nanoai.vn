...
export async function GET(request: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    if (!isPgTransientError(error)) throw error
    const empty = emptyFeaturedCategoryBlock(slug)
    return NextResponse.json({
      ok: true,
      tiles: empty.tiles,
      nav_pills: empty.nav_pills,
      nav_row: empty.nav_row,
      show_nav_all: empty.show_nav_all,
      gender: empty.gender,
      gender_label: empty.gender_label,
      source: empty.source,
