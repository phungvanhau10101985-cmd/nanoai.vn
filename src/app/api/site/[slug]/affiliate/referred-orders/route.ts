...
import {
  ...
  listPartnerAffiliateReferredOrdersFromPg,
...
export async function GET(request: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const orders = await listPartnerAffiliateReferredOrdersFromPg({
    partnerId: store.shop.partnerId,
    profileId: profile.id,
    skip,
    limit,
  })
  return jsonAffiliateStorefront(request, store, { ok: true, orders })
}
