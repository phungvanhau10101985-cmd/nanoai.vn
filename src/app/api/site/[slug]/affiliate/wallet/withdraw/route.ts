...
import {
  ...
  requestPartnerAffiliateWithdrawalFromPg,
...
export async function GET(request: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const withdrawals = await listPartnerAffiliateWithdrawalsFromPg({
    partnerId: store.shop.partnerId,
    profileId: profile.id,
    skip,
    limit,
  })
  return jsonAffiliateStorefront(request, store, { ok: true, withdrawals })
}
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const body = (await request.json().catch(() => null)) as { amount?: number; otp?: string } | null
  try {
    const withdrawal = await requestPartnerAffiliateWithdrawalFromPg({
      ...store.identity,
