...
export async function POST(request: Request) {
  try {
    ...
    const ct = file.type || 'audio/wav'
    if (!allowed.includes(ct)) {
      return NextResponse.json({ error: 'Chỉ hỗ trợ WAV hoặc MP3.' }, { status: 400 })
    }
    ...
    const uploadPath = `music-history/${user.id}/music_${timestamp}.${ext}`
    const buffer = Buffer.from(await file.arrayBuffer())
    const { error: uploadError } = await adminSupabase.storage
      .from('try-on-images')
      .upload(uploadPath, buffer, { contentType: ct, upsert: true })
