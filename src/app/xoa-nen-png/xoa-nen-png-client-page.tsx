...
export default function XoaNenPngClientPage() {
  ...
  const handleSubmit = async () => {
    if (!image.file) {
      toast({ title: 'Lỗi', description: 'Vui lòng tải lên ảnh cần xóa nền.', variant: 'destructive' })
      return
    }
    setStep('GENERATING')
    ...
    if (result.error) {
      setStep('UPLOAD')
      toast({ title: 'Xóa nền thất bại', description: result.error, variant: 'destructive', duration: 5000 })
    } else if (result.success && result.resultUrl) {
      setResultUrl(result.resultUrl)
      setStep('RESULT')
      toast({ title: 'Thành công!', description: 'Ảnh PNG nền trong suốt đã sẵn sàng.', duration: 3000 })
    }
  }
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
              ...
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-teal-200/60">
                <CardHeader className="p-4 pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Upload className="h-4 w-4 text-teal-600" /> Tải ảnh cần xóa nền
                  </CardTitle>
            ...
            <div className="lg:w-[220px] lg:shrink-0">
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-teal-200/60 h-full">
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardDescription className="text-xs">Chọn chất lượng ảnh đầu ra.</CardDescription>
                </CardHeader>
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="pt-4 border-t space-y-2 flex flex-col items-center">
                    ...
                    >
                      <Sparkles className="mr-2 h-4 w-4" /> Xóa nền ({imageQuality === '2K' ? '1,5' : '3'} credit)
                    </Button>
                    <p className="text-[10px] text-center text-muted-foreground mt-2">* Trả ảnh PNG nền trong suốt</p>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <Card className="border shadow-sm bg-white/80 backdrop-blur border-teal-200/60">
            <CardContent className="flex flex-col items-center py-8">
              <ImageProcessingLoader
                mode="eraser"
                title="Đang tách nền PNG"
                description="AI đang tạo mask và xuất ảnh nền trong suốt (alpha)"
        ...
        {step === 'RESULT' && resultUrl && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur">
            <CardHeader>
              <CardTitle>Kết quả PNG trong suốt</CardTitle>
              <CardDescription>Ảnh đã được tách nền thành công.</CardDescription>
            </CardHeader>
            <CardContent className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium text-muted-foreground">Sau (PNG alpha)</h3>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={handleReset}>
                      <RefreshCw className="mr-2 h-3 w-3" /> Thử lại
                    </Button>
                    <DownloadImageButton imageUrl={resultUrl} filename="xoa-nen-png-result" size="sm" className="bg-teal-600 hover:bg-teal-700 text-white border-0" />
                  </div>
                </div>
                <div className="aspect-square rounded-lg border overflow-hidden bg-[linear-gradient(45deg,#eee_25%,transparent_25%,transparent_75%,#eee_75%,#eee),linear-gradient(45deg,#eee_25%,transparent_25%,transparent_75%,#eee_75%,#eee)] bg-[length:24px_24px] bg-[position:0_0,12px_12px]">
                  <ImagePreview src={resultUrl} alt="Sau" className="w-full h-full object-contain" />
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <p className="text-xs text-muted-foreground text-center mt-6">Ảnh PNG trong suốt phù hợp để ghép nền, thiết kế và đăng sản phẩm lên sàn TMĐT.</p>
    </>
  )
}
