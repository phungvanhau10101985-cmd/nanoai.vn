...
export async function create3DMockup(formData: FormData) {
  ...
  const { publicUrl: logoPublicUrl } = await uploadTryOnImagePublic(logoPath, logoImage, {
  ...
  const historyItem = await insertTryOnHistoryProcessingPg({
    userId: user.id,
    originalImageUrl: logoPublicUrl,
    garmentImageUrl: logoPublicUrl,
    feature: 'tao-anh-3d',
  })
  ...
  try {
    ...
    trackFromUsageMetadata(response.usageMetadata, GEMINI_3_PRO_IMAGE.model, 'tao-anh-3d', user.id, imageQuality)
