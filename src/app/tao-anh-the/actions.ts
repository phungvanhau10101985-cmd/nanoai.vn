...
export async function createIdCard(formData: FormData) {
  ...
  try {
    ...
    trackFromUsageMetadata(response.usageMetadata, GEMINI_3_PRO_IMAGE.model, 'tao-anh-the', user.id, imageQuality)
    ...
    const d = await deductUserCredits(user.id, COST, 'tao-anh-the')
    if (!d.ok) {
      await deleteTryOnHistoryRowAndStorage(historyItem.id)
