...
import { Upload, Sparkles, Download, RefreshCw, Link2, Box } from 'lucide-react'
...
export default function TaoMoHinh3DTuAnhClientPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!modelStyle) {
      toast({ title: 'Lỗi', description: 'Vui lòng chọn định dạng mô hình.', variant: 'destructive' })
      return
    }
    setStep('GENERATING')
    ...
    if (result.error) {
      setStep('UPLOAD')
      toast({ title: 'Tạo mô hình 3D thất bại', description: result.error, variant: 'destructive', duration: 5000 })
    } else if (result.success && result.resultUrl) {
      setResultUrl(result.resultUrl)
      setStep('RESULT')
      toast({ title: 'Thành công!', description: 'Đã tạo mô hình 3D từ ảnh.', duration: 3000 })
    }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground flex items-center justify-center gap-2">
            <Box className="h-8 w-8 text-amber-600" /> Tạo mô hình 3D từ ảnh
          </h1>
          <p className="text-muted-foreground mt-1">Chuyển ảnh thành mô hình 3D dạng hoạt hình. AI tạo mô hình mới, không giữ nguyên ảnh gốc.
          </p>
        </div>
              ...
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-amber-200/60">
                <CardHeader className="p-4 pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Upload className="h-4 w-4 text-amber-600" /> Ảnh 2D gốc
                  </CardTitle>
                  <CardDescription className="text-xs">Tải lên ảnh tham khảo. AI tạo mô hình 3D mới theo phong cách bạn chọn.</CardDescription>
                </CardHeader>
                <CardContent className="p-4 pt-0 space-y-4">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Định dạng mô hình</h4>
                    <select
                      value={modelStyle}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input placeholder="VD: góc isometric, phát sáng nhẹ..." value={note} onChange={(e) => setNote(e.target.value)} className="bg-white/80" />
                  </div>
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="pt-4 border-t space-y-2 flex flex-col items-center">
                    ...
                    <Button onClick={() => checkCreditsAndProceed(cost, handleSubmit)} disabled={!image.file} className="w-full max-w-[180px] h-9 shadow-md hover:shadow-lg transition-all text-sm bg-amber-600 hover:bg-amber-700 text-white">
                      <Sparkles className="mr-2 h-4 w-4" /> Tạo mô hình 3D ({imageQuality === '2K' ? '1,5' : '3'} credit)
                    </Button>
        ...
        {step === 'GENERATING' && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur border-amber-200/60">
            <CardContent className="flex flex-col items-center py-8">
              <ImageProcessingLoader mode="model3d" title="Đang tạo mô hình 3D" description="AI đang chuyển ảnh thành preview 3D" imagePreview={image.preview} />
            </CardContent>
          </Card>
        )}
        ...
        {step === 'RESULT' && resultUrl && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription>Đã tạo mô hình 3D từ ảnh.</CardDescription>
            </CardHeader>
            <CardContent className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-muted-foreground">Ảnh tham khảo</h3>
                {image.preview && (
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium text-muted-foreground">Mô hình 3D</h3>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={handleReset}><RefreshCw className="mr-2 h-3 w-3" /> Thử lại</Button>
                    <DownloadImageButton imageUrl={resultUrl} filename="mo-hinh-3d-result" size="sm" className="bg-amber-600 hover:bg-amber-700 text-white border-0" />
                  </div>
                </div>
                <div className="aspect-square rounded-lg border overflow-hidden">
                  <ImagePreview src={resultUrl} alt="Mô hình 3D" className="w-full h-full object-cover" />
                </div>
              </div>
            </CardContent>
          </Card>
