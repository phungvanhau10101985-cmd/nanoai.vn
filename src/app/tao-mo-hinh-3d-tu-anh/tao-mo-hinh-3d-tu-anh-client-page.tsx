    ...
    try {
      ...
      await finalizeStandardImageGenerationResult(result, {
        onServerErrorMessage: (message) => {
          setStep('UPLOAD')
          toast({
            title: tr('Tạo mô hình 3D thất bại', 'Create 3D model failed', '创建 3D 模型失败', '3Dモデル作成に失敗しました', '3D 모델 생성 실패'),
            description: message,
            variant: 'destructive',
            duration: 5000,
          })
        ...
        onSuccessWithUrl: (url) => {
          ...
          toast({
            ...
            description: tr('Đã tạo mô hình 3D từ ảnh.', '3D model created from image.', '已从图片创建 3D 模型。', '画像から3Dモデルを作成しました。', '이미지에서 3D 모델이 생성되었습니다.'),
            duration: 3000,
          })
        },
        onUnexpectedPayload: () => {
