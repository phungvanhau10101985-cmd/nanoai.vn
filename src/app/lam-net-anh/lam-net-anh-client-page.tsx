...
export default function LamNetAnhClientPage() {
  ...
  const handleSubmit = async () => {
    if (!image.file) {
      toast({ title: 'Lỗi', description: 'Vui lòng tải lên ảnh cần làm nét.', variant: 'destructive' })
      return
    }
    setStep('GENERATING')
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground">Làm nét ảnh</h1>
          <p className="text-muted-foreground mt-1">Tăng độ sắc nét, giảm mờ với AI. 1,5–3 credits/ảnh.</p>
        </div>
              ...
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-amber-200/60">
                <CardHeader className="p-4 pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Upload className="h-4 w-4 text-amber-600" /> Tải ảnh cần làm nét
                  </CardTitle>
              ...
              <Card className="border shadow-sm bg-white/80 backdrop-blur border-amber-200/60 h-full">
                ...
                <CardContent className="p-4 space-y-4">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      placeholder="Ví dụ: ưu tiên làm nét khuôn mặt, văn bản, chữ..."
                      value={note}
                  ...
                  <div className="pt-4 border-t space-y-2 flex flex-col items-center">
                    ...
                    >
                      <Sparkles className="mr-2 h-4 w-4" /> Làm nét ({imageQuality === '2K' ? '1,5' : '3'} credit)
                    </Button>
          ...
          <Card className="border shadow-sm bg-white/80 backdrop-blur border-amber-200/60">
            <CardContent className="flex flex-col items-center py-8">
              <ImageProcessingLoader
                mode="sharpen"
                title="Đang làm nét ảnh"
                description="AI đang tăng độ sắc nét và chi tiết để ảnh rõ ràng hơn"
        ...
        {step === 'RESULT' && resultUrl && (
          <Card className="border shadow-sm bg-white/80 backdrop-blur">
            <CardHeader>
              <CardTitle>Kết quả làm nét</CardTitle>
