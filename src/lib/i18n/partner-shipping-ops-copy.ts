...
const COPY: Record<WebLocale, PartnerShippingOpsCopy> = {
  vi: {
    ...
    importEmsHint:
      'File gui ems.xlsx (hoặc .xls): cột A mã vận đơn, I mã đơn shop, G COD, D tên khách. Import lần 2: mã cột A đã có thì cập nhật. Sau import tự tra MyEMS nền; xem báo cáo từng dòng bên dưới.',
  ...
  en: {
    ...
    importEmsHint: 'gui ems.xlsx / .xls: A tracking, I shop order, G COD, D name. Re-import updates existing A codes. MyEMS tracking starts automatically; see the row report below.',
  ...
  zh: {
    ...
    importEmsHint: 'gui ems.xlsx / .xls：A 运单、I 订单、G COD、D 姓名。再次导入会更新已有 A 码。导入后自动查询 MyEMS，见下方逐行报告。',
  ...
  ja: {
    ...
    importEmsHint: 'gui ems.xlsx / .xls：A 追跡、I 注文、G 代引、D 氏名。再取込は既存 A を更新。取込後 MyEMS を自動照会し、下の行レポートを確認。',
  ...
  ko: {
    ...
    importEmsHint: 'gui ems.xlsx / .xls: A 운송장, I 주문, G COD, D 이름. 재가져오기는 기존 A를 갱신. 가져온 뒤 MyEMS를 자동 조회하며 아래 행 보고서를 봅니다.',
