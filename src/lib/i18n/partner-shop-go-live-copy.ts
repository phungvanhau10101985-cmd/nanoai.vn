...
const COPY: Record<WebLocale, PartnerGoLiveCopy> = {
  vi: {
    ...
    items: {
      ...
      ads: { label: 'Pixel quảng cáo', hint: 'Đã dán ID thì live HTML (home/PDP/listing) phải bắn Meta / GA4 / TikTok / GTM — không chỉ trang React giỏ.' },
  ...
  en: {
    ...
    items: {
      ...
      ads: { label: 'Ad pixels', hint: 'After you paste IDs, live HTML (home/PDP/listing) must fire Meta / GA4 / TikTok / GTM — not only the React cart.' },
  ...
  zh: {
    ...
    items: {
      ...
      ads: { label: '广告像素', hint: '填入 ID 后，线上 HTML（首页/商品/列表）必须触发 Meta / GA4 / TikTok / GTM，不只是购物车 React 页。' },
  ...
  ja: {
    ...
    items: {
      ...
      ads: { label: '広告ピクセル', hint: 'ID を保存したらライブ HTML（ホーム/商品/一覧）で Meta / GA4 / TikTok / GTM が発火すること。カートの React ページだけではありません。' },
  ...
  ko: {
    ...
    items: {
      ...
      ads: { label: '광고 픽셀', hint: 'ID를 붙여 넣은 뒤에는 라이브 HTML(홈/PDP/목록)에서 Meta / GA4 / TikTok / GTM이 발사되어야 합니다. React 장바구니만으로는 부족합니다.' },
