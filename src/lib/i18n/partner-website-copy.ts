...
const COPY: Record<WebLocale, PartnerWebsiteCopy> = {
  vi: {
    ...
    visualEditChromeKitFloatHint:
      'Bật mắt để hiện. Một cột khóa góc màn — gõ Cách phải / Cách dưới / Khoảng cách. Mỗi nút một cỡ (ô px trên hàng đó). Danh sách trên xuống dưới = cột góc màn. ↑ lên trên cột, ↓ xuống dưới. Ẩn thì nút còn lại khít lại. Bấm tên để sửa từng nút. Mỗi máy một bản.',
    ...
    visualEditChromeKitFloatSizeHint: 'Mỗi nút một cỡ. Gõ số trên hàng đó, hoặc bấm tên để sửa trên canvas.',
  ...
  en: {
    ...
    visualEditChromeKitFloatHint:
      'Turn the eye on to show. One locked corner column — type Right / Bottom / Gap. Each button has its own size (px box on that row). The list top-to-bottom matches the corner column. Up moves toward the top of the column. Hidden buttons close the gap. Click a name to edit that button. One set per device.',
    visualEditChromeKitFloatLocked:
    ...
    visualEditChromeKitFloatSizeHint: 'Each button has its own size. Type the px on that row, or click the name to edit it on the canvas.',
  ...
  zh: {
    ...
    visualEditChromeKitFloatHint: '点眼睛显示。角落一列锁定 — 输入距右 / 距下 / 间距。每个按钮单独尺寸（该行的 px 框）。列表从上到下与角落列一致。↑ 上移，↓ 下移。隐藏后其余按钮靠拢。点名称可单独编辑。每台设备一份。',
    ...
    visualEditChromeKitFloatSizeHint: '每个按钮单独尺寸。在该行输入 px，或点名称在画布上修改。',
  ...
  ja: {
    ...
    visualEditChromeKitFloatHint:
      '目のアイコンで表示。角の1列に固定 — 右・下・間隔を入力。サイズはボタンごとにその行の px。リスト上から下は角の列と同じ。↑は上、↓は下。非表示にすると残りが詰まります。名前を押すと個別編集。端末ごとに別です。',
    visualEditChromeKitFloatLocked:
    ...
    visualEditChromeKitFloatSizeHint: 'ボタンごとにサイズが違います。その行の px を入力するか、名前を押してキャンバスで編集します。',
  ...
  ko: {
    ...
    visualEditChromeKitFloatHint:
      '눈을 켜면 보입니다. 모서리 한 열로 고정 — 오른쪽/아래/간격을 입력하세요. 크기는 버튼마다 그 행의 px. 목록 위→아래가 모서리 열과 같습니다. ↑는 위로, ↓는 아래로. 숨기면 나머지 버튼이 붙습니다. 이름을 누르면 개별 편집. 기기마다 따로입니다.',
    visualEditChromeKitFloatLocked:
    ...
    visualEditChromeKitFloatSizeHint: '버튼마다 크기가 다릅니다. 그 행에 px를 입력하거나 이름을 눌러 캔버스에서 수정합니다.',
