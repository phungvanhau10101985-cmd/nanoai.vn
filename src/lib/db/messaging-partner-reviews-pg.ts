...
const REVIEW_SELECT = `id::text, partner_id::text, inventory_id::text, order_id::text, order_line_id::text,
  ...
  merchant_reply_at, created_at, updated_at`

const QUESTION_SELECT = `id::text, partner_id::text, inventory_id::text, guest_account_id::text,
  linked_user_id::text, coalesce(asker_name, '') as asker_name, content, is_active, created_at, updated_at`
...
/**
 * Trang review công khai cho 1 sản phẩm — thứ tự: review của chính khách đang xem → còn lại theo
 * useful_count desc → ngày mới nhất (xem 188_BEHAVIOR_SPEC.md mục C.1, không copy "cố định import
 * trước" của 188 vì NanoAI web không có review import).
 */
  ...
  try {
    const rows = await pgQuery<ReviewDbRow>(
      `select ${REVIEW_SELECT}
       ...
       where partner_id = $1::uuid and inventory_id = $2::uuid and is_active = true
         ${ratingFilter ? 'and rating = $5' : ''}
       order by
      ...
      ratingFilter
        ? [input.partnerId, input.inventoryId, viewerKey, pageSize, ratingFilter]
        : [input.partnerId, input.inventoryId, viewerKey, pageSize]
    )
    const totalRow = await pgQueryOne<{ c: number }>(
      ...
      ratingFilter ? [input.partnerId, input.inventoryId, ratingFilter] : [input.partnerId, input.inventoryId]
    )
...
/** Admin (M1.2) — phân trang 10/dòng, lọc theo nhóm rating, gồm cả inactive. */
...
/** Danh sách câu hỏi công khai của 1 sản phẩm, kèm câu trả lời (buyer giới hạn slot + admin riêng). */
export async function fetchPartnerProductQuestionsPageFromPg(input: {
  ...
  try {
    const questions = await pgQuery<QuestionDbRow>(
      ...
      [input.partnerId, input.inventoryId, pageSize]
    )
    const totalRow = await pgQueryOne<{ c: number }>(
      `select count(*)::int as c from public.messaging_partner_product_questions
       where partner_id = $1::uuid and inventory_id = $2::uuid and is_active = true`,
      [input.partnerId, input.inventoryId]
    )
