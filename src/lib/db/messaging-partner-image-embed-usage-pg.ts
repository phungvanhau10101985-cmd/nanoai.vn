...
export type PartnerImageEmbedUsageSummaryRow = {
  source: PartnerImageEmbedSource
  call_count: number
  sum_prompt_tokens: number
  sum_total_tokens: number
}

export type PartnerImageEmbedUsageDetailRow = {
  id: string
  created_at: string
  source: PartnerImageEmbedSource
  model: string
...
export async function insertMessagingPartnerImageEmbedUsageFromPg(params: {
  partnerId: string
  source: PartnerImageEmbedSource
  model: string
  promptTokens: number
  ...
  try {
    await pgQuery(
      `insert into public.messaging_partner_image_embed_usage (
    ...
    console.warn('[insertMessagingPartnerImageEmbedUsageFromPg]', e)
    return false
  }
}
...
export async function fetchMessagingPartnerImageEmbedStatsBySourceFromPg(
  ...
  try {
    ...
    console.warn('[fetchMessagingPartnerImageEmbedStatsBySourceFromPg]', e)
    return null
  }
}
...
export async function fetchMessagingPartnerImageEmbedDetailsFromPg(
  ...
  try {
    ...
    const code = typeof e === 'object' && e && 'code' in e ? String((e as { code?: unknown }).code || '') : ''
    if (code === '42P01') return []
    console.warn('[fetchMessagingPartnerImageEmbedDetailsFromPg]', e)
    return null
  }
}
