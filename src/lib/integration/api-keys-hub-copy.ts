...
export const PARTNER_API_KEYS_MANAGER_COPY: Record<ApiKeysHubLocale, PartnerApiKeysManagerStrings> = {
  vi: {
    ...
    embedTitle: 'X-Embed-Key (nhúng chat & đăng nhập tự động)',
    embedHint:
      'Đặt trên server shop (ví dụ `NANOAI_EMBED_KEY`): header X-Embed-Key cho API embed và ký token đăng nhập tự động. Không đưa vào JS trình duyệt khách.',
  ...
  en: {
    ...
    embedTitle: 'X-Embed-Key (chat embed & auto sign-in)',
    embedHint:
      'Keep on your shop server (e.g. `NANOAI_EMBED_KEY`): X-Embed-Key header for embed API and partner auto sign-in token signing. Never in shopper-facing JS.',
  ...
  zh: {
    ...
    embedTitle: 'X-Embed-Key（聊天嵌入与自动登录）',
    embedHint:
      '放在店铺服务端（如 `NANOAI_EMBED_KEY`）：嵌入 API 的 X-Embed-Key 请求头及自动登录 token 签名。勿放入买家浏览器 JS。',
  ...
  ja: {
    ...
    embedTitle: 'X-Embed-Key（チャット埋め込み・自動ログイン）',
    embedHint:
      '店舗サーバーに保持（例: `NANOAI_EMBED_KEY`）: 埋め込み API の X-Embed-Key と自動ログイン token 署名。購入者向け JS には含めない。',
  ...
  ko: {
    ...
    embedTitle: 'X-Embed-Key(채팅 임베드·자동 로그인)',
    embedHint:
      '매장 서버에 보관(예: `NANOAI_EMBED_KEY`): 임베드 API X-Embed-Key 헤더 및 자동 로그인 token 서명. 구매자 JS에 넣지 마세요.',
