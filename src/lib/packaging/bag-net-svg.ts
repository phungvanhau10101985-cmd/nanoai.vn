...
const BAG_NET_PAD_MM = 2
const BAG_NET_GAP_MM = 0

export type BagPrintPanelSlot = 'back' | 'front'

export type BagPrintPanelRect = { xMm: number; yMm: number; widthMm: number; heightMm: number }

/** Printable back/front panel positions on the flat bag net (mm). */
export function getBagPrintPanelRects(d: BagDimensions): Record<BagPrintPanelSlot, BagPrintPanelRect> {
  const { widthMm: W, heightMm: H, gussetMm: G } = d
  const pad = BAG_NET_PAD_MM
  const gap = BAG_NET_GAP_MM
  return {
    back: { xMm: pad, yMm: pad, widthMm: W, heightMm: H },
    front: { xMm: pad + W + gap + G + W + gap + G, yMm: pad, widthMm: W, heightMm: H },
  }
}
