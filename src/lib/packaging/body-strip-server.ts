...
import { fitPanelArtworkToTrim } from './panel-artwork-fit'

export async function splitBodyStripBuffer(
  buffer: Buffer,
  dimensions: BoxDimensionsMm
): Promise<Partial<Record<BoxFaceSlot, Buffer>>> {
  const { widthMm, heightMm } = getBodyStripSizeMm(dimensions)
  const normalized = await fitPanelArtworkToTrim(buffer, widthMm, heightMm)
  ...
  for (let index = 0; index < segments.length; index++) {
    ...
    result[segment.slot] = await sharp(normalized)
