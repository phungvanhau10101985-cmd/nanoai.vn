...
/** Khớp 188 `import_1688_scraper._extract_body_specs`. */
export function extract1688BodySpecs(bodyText: string): {
  ...
  if (colorM) {
    const raw = colorM[1].trim().split('\n')[0]
    out.colors_label = dedupe(raw.split(/[、,，+/]\s*/).map((p) => p.trim()).filter(Boolean)).slice(0, 30)
  }
  const sizeM = bodyText.match(/尺码[：:\t\s]*([0-9A-Za-z\s，、,.-]+)/)
  if (sizeM) {
    const raw = sizeM[1].trim().split('\n')[0]
    out.sizes_label = dedupe(raw.split(/[、,，/\s]+\s*/).map((p) => p.trim()).filter(Boolean)).slice(0, 60)
  }
...
/** Bảng thuộc tính 1688 trên Vipomall (nhãn một dòng, giá trị dòng sau). */
export function extractCjkAttributePairs(bodyText: string): Record<string, string> {
  ...
  const lines = section
    ...
    .filter(Boolean)
  const keyRe = /^([\u3400-\u9fffA-Za-z0-9/%％（）()._-]{1,40})\s*[:：]\s*(.*)$/
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!val && i + 1 < lines.length) {
      const nxt = lines[i + 1]
      const nxtIsBareKey = /^[\u3400-\u9fffA-Za-z0-9/%％（）()._-]{1,40}\s*[:：]\s*$/.test(nxt)
      if (!nxtIsBareKey && !/^(Xem thêm|Thu gọn|Đóng)$/i.test(nxt)) {
        val = nxt
        i += 1
      }
    }
    if (!key || !val || /^(Xem thêm|Thu gọn|Đóng)$/i.test(val)) continue
    if (!/[\u3400-\u9fff]/.test(key) && key !== 'S' && !/货号|尺码|颜色/.test(key)) continue
...
/**
 * Gắn bảng thông số 1688 vào `product_info` + mô tả — giống `normalize_1688_payload`.
 */
export function enrichListingProductDataFromBody(
  ...
  const articleNo = bodySpecs.article_no || String(pairs['货号'] || '').trim()

  const pi = asRecord(productData.product_info)
  const inner = asRecord(pi.product_info)
  const spec = asRecord(pi.specifications)

  if (articleNo) inner.article_no = articleNo.slice(0, 120)
  for (const [k, v] of Object.entries(pairs)) {
    if (!spec[k]) spec[k] = v
  }
  if (bodySpecs.article_no && !spec['货号']) spec['货号'] = bodySpecs.article_no
  if (bodySpecs.sizes_label.length && !spec['尺码']) spec['尺码'] = bodySpecs.sizes_label.join(',')
  if (bodySpecs.colors_label.length && !spec['颜色']) spec['颜色'] = bodySpecs.colors_label.join('、')
  ...
  if (specLines.length) {
    ...
    if (!/货号\s*[:：]/.test(desc) && !/材质成分/.test(desc)) {
