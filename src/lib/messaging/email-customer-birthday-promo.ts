...
export async function emailCustomerBirthdayPromo(input: {
  ...
  const text = [
    `Xin chào,`,
    ``,
    `${input.shopDisplayName} gửi lời chúc mừng sinh nhật — trong khung ưu đãi, giá các sản phẩm trong kho trên chat được giảm ${pct}% tự động (không cần mã).`,
...
<p>Xin chào,</p>
<p><strong>${safeShop}</strong> gửi lời chúc mừng sinh nhật — trong khung ưu đãi, giá sản phẩm trong kho trên chat giảm <strong>${pct}%</strong> tự động, không cần mã giảm giá.</p>
