 ...
 */
export async function deliverAutomatedPartnerMessage(
  db: Db,
  params: {
  ...
  if (conversation.channel === 'facebook') {
    ...
    if (pageId) {
      const r = await getFacebookSendToken(db, conversation.partner_id, pageId)
    ...
    const r = await getZaloSendToken(db, conversation.partner_id)
    if (r.error) return { error: r.error }
