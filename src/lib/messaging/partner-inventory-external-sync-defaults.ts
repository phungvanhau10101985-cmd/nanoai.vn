...
export const INVENTORY_EXTERNAL_SYNC_MAP_KEYS = [
  ...
  'remarketing_id',
  'name',
  /** Cột NanoAI `description` thực chất lưu JSON mảng size (dùng cho bộ chọn size trên shop). */
  'description',
  'price',
  'stock_qty',
  /** JSON mảng [{ name, img }] — ghi thẳng vào cột `stock_note` (dùng cho bộ chọn màu trên shop). */
  'colors_json',
  'image',
  'slug',
  'video',
...
export const DEFAULT_188_INVENTORY_FIELD_MAPPING: Record<InventoryExternalSyncMapKey, string> = {
  ...
  name: 'name',
  /** `description` (NanoAI) = size — map tới `sizes` (188), KHÔNG map tới `description` (188). */
  description: 'sizes',
  price: 'price',
  stock_qty: 'available',
  colors_json: 'colors',
