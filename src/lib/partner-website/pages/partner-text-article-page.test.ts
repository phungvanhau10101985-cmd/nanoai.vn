...
test('pageKey shipping/returns/faq are text articles; sale/home are not', () => {
  ...
  assert.equal(isPartnerTextArticlePageKey('how_to_buy'), true)
  assert.equal(isPartnerTextArticlePageKey('how-to-buy'), true)
  assert.equal(isPartnerTextArticlePageKey('company'), true)
...
<article data-pw-region="content"><h1>Đổi trả</h1><p>Nội dung</p></article></body></html>`
  ...
  assert.equal(resolvePartnerTextArticleKind({ pageKey: 'how-to-buy' }), 'guide')
  assert.equal(resolvePartnerTextArticleKind({ pageKey: 'how_to_buy' }), 'guide')
