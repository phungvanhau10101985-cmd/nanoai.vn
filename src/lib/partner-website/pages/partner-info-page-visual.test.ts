...
test('default privacy/terms copy already includes ads platform policy', () => {
  ...
  const howToBuy = getPartnerSiteInfoPage('how-to-buy', 'vi')
  assert.ok(contentHasAdsPlatformPolicy(howToBuy.paragraphs.join('\n')))
  assert.doesNotMatch(howToBuy.paragraphs.join('\n'), /30\s*%/)
  const origin = getPartnerSiteInfoPage('brand-origin', 'vi')
  assert.ok(contentHasAdsPlatformPolicy(origin.paragraphs.join('\n')))
  const reviews = getPartnerSiteInfoPage('reviews-policy', 'vi')
  assert.match(reviews.title, /đánh giá/i)
  const trust = getPartnerSiteInfoPage('trust', 'vi')
  assert.match(trust.title, /uy tín/i)
  const company = getPartnerSiteInfoPage('company', 'vi')
  assert.match(company.title, /đơn vị/i)
