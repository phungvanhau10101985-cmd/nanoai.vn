...
test('stamps featured category tiles for personalization', () => {
  ...
  assert.match(html, /data-pw-el="card-name"/)
  assert.doesNotMatch(html, /pw-featured-cat-tile-shine/)
  assert.match(html, /\/c"/)
...
test('stamps recently viewed and recommended personalize hooks', () => {
  ...
  assert.match(rec, /data-pw-rec-head/)
  assert.match(rec, /pw-rec-title/)
  assert.match(rec, /pw-rec-badge/)
  assert.match(rec, /pw-rec-stats/)
  assert.match(rec, /Đề xuất/)
  assert.match(rec, /Xem thêm/)
  assert.doesNotMatch(rec, /Xem tất cả các nhóm/)
  assert.doesNotMatch(rec, /data-pw-el="section-more"/)
...
test('stamps flash sale personalize hook like 188', () => {
  ...
  assert.doesNotMatch(html, /data-pw-flash-sub/)
  assert.doesNotMatch(html, /12 deal \/ 10 phút/)
