...
describe('pw scene layers', () => {
  ...
  it('pins each layer to the same fixed width as the Sửa nhanh iframe', () => {
    ...
    expect(PARTNER_SHOP_IMAGE_ZOOM_SCRIPT).toContain('function livePromoHost(el){')
    expect(PARTNER_SHOP_IMAGE_ZOOM_SCRIPT).toContain('data-pw-promo-carousel')
    expect(PARTNER_SHOP_IMAGE_ZOOM_SCRIPT).toContain('if(livePromoHost(host))return;')
    expect(PW_SCENE_MEDIA_ZOOM_SEL).toContain('.pw-hero img[data-pw-el="media"]')
    expect(PW_SCENE_MEDIA_ZOOM_SEL).not.toContain('.pw-hero img:not(.pw-logo)')
    ...
    expect(PARTNER_SHOP_SCENE_CENTER_SCRIPT).toContain('[data-pw-chrome-kit="dock"]')
    expect(PARTNER_SHOP_SCENE_CENTER_SCRIPT).toContain('var host=document.body;')
    expect(PARTNER_SHOP_SCENE_CENTER_SCRIPT).toContain("document.querySelector('[data-pw-live-dock=\"1\"]')")
...
describe('scene layers inside the editor runtime', () => {
  ...
  it('sizes added backgrounds and banners from edge handles and the floating panel', () => {
    ...
    expect(script).toContain("if (isBannerPhotoTarget(el) || isBannerHostEl(el) || bannerHostOf(el)) return []")
    expect(script).toContain('pwBannerFitAll(true)')
    expect(script).toContain('function pwBannerClearAuthoredSize(')
    expect(script).toContain('isBanner: Boolean(isBannerHostEl(el)')
  ...
  it('writes scene on the chrome widget itself and only lifts chrome dragged off header', () => {
    ...
    expect(script).toContain('liftLooseElToSceneHost(unit)')
  ...
  it('keeps scene-absolute overlays on the scene-root origin instead of viewport 0', () => {
    ...
    expect(PARTNER_SHOP_STACK_FLOW_CSS).toContain('[data-pw-trust-bar="1"]')
    expect(PARTNER_SHOP_STACK_FLOW_CSS).toContain('.pw-marketplace-trust-item')
    expect(PARTNER_SHOP_STACK_FLOW_CSS).toContain('.pw-featured-cat')
    expect(PARTNER_SHOP_STACK_FLOW_CSS).toContain('.pw-section')
    ...
    expect(PARTNER_SHOP_STACK_FLOW_CSS).toContain('html .pw-container.pw-lead-inner{max-width:560px!important')
    expect(PARTNER_SHOP_STACK_FLOW_CSS).toContain('html .pw-marketplace-trust,html [data-pw-trust-bar="1"]{display:grid!important}')
    ...
    expect(PARTNER_SHOP_SCENE_CENTER_SCRIPT).toContain("querySelectorAll('[data-pw-live-chrome]')")
    expect(PARTNER_SHOP_SCENE_CENTER_SCRIPT).toContain('if(!liveRoot()){')
  })
})
