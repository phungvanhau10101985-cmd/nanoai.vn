...
const RUNTIME_BODY = `(function (MSG, COPY, SCENE) {
  ...
  var PW_SHOP_DOCK_TRYON_HTML = ${JSON.stringify(shopDockTryOnHtmlByLocale())}
  ...
  function syncEditDeviceAttr() {
    ...
    var sceneW = ${PW_SCENE_CANVAS_WIDTH.desktop}
    if (stamped === 'mobile') sceneW = ${PW_SCENE_CANVAS_WIDTH.mobile}
    else if (stamped === 'tablet') sceneW = ${PW_SCENE_CANVAS_WIDTH.tablet}
    else if (stamped === 'laptop') sceneW = ${PW_SCENE_CANVAS_WIDTH.laptop}
    try {
      document.documentElement.style.setProperty('--pw-scene-w', sceneW + 'px')
      document.documentElement.style.setProperty('--pw-scene-zoom', '1')
      document.documentElement.removeAttribute('data-pw-scene-zoomed')
    } catch (errSceneW) {}
    if (document.body) {
  ...
  var editorChromeFitTimer = 0
  function scheduleEditorChromeFit() {
    var run = function () {
      try { syncEditDeviceAttr() } catch (errDevFit) {}
      try { reflowHeaderChrome() } catch (errHeadFit) {}
      try { lockExistingSearchBoxes() } catch (errSearchFit) {}
    }
    run()
    if (window.requestAnimationFrame) window.requestAnimationFrame(run)
    try { window.clearTimeout(editorChromeFitTimer) } catch (errFitT) {}
    editorChromeFitTimer = window.setTimeout(run, 80)
  }
  ...
  function defaultLogoFrameBox() {
    if (editDevice === 'mobile') return { w: 72, h: 28 }
    if (editDevice === 'tablet') return { w: 96, h: 32 }
    return { w: 140, h: 36 }
  }
  ...
  function applyLogoFrameSize(frame, w, h, user) {
    ...
    frame.style.setProperty('--pw-logo-box-w', box.w + 'px', 'important')
    frame.style.setProperty('--pw-logo-box-h', box.h + 'px', 'important')
  ...
  function ensureLogoFrame(img) {
    ...
    wrap.style.cssText = 'display:inline-flex;align-items:center;justify-content:center;overflow:hidden;width:' + w + 'px;height:' + h + 'px;--pw-logo-box-w:' + w + 'px;--pw-logo-box-h:' + h + 'px;max-width:none;max-height:none;flex-shrink:0;vertical-align:middle;position:relative;z-index:${PW_SCENE_LOGO_Z}'
    ...
    img.style.height = '100%'
    img.style.objectFit = 'contain'
    applyLogoFrameSize(wrap, w, h, wrap.getAttribute('data-pw-logo-user-size') === '1')
  ...
  function compactSearchMinWidth() {
    return editDevice === 'tablet' ? 180 : 96
  }
  ...
  function chromeKitHeaderOf(host) {
    if (host && host.closest) {
      return host.closest('header,.pw-header,.pw-shop-header,[data-pw-region="header"]')
    }
    return scopedQuery('header.pw-header, header.pw-shop-header, header')
  }
  function stampChromeKitOff(el, kit) {
    if (!el || !el.setAttribute || !kit) return
    var parts = String(el.getAttribute('data-pw-kit-off') || '').split(/\\s+/).filter(Boolean)
    if (parts.indexOf(kit) < 0) parts.push(kit)
    el.setAttribute('data-pw-kit-off', parts.join(' '))
  }
  function clearChromeKitOff(el, kit) {
    if (!el || !kit) return
    var parts = String(el.getAttribute('data-pw-kit-off') || '').split(/\\s+/).filter(function (p) { return p && p !== kit })
    if (parts.length) el.setAttribute('data-pw-kit-off', parts.join(' '))
    else try { el.removeAttribute('data-pw-kit-off') } catch (errOff) {}
  }
  function revealChromeKitTopbarHost() {
    var host = chromeKitTopbarRoot()
    if (host) {
      try { host.removeAttribute('data-pw-hidden') } catch (errHostHid) {}
      if (host.style) {
    ...
    var header = chromeKitHeaderOf(host)
    if (header) clearChromeKitOff(header, 'topbar')
  }
  function hideChromeKitTopbarHost(host) {
    if (!host || !host.setAttribute) return
    host.setAttribute('data-pw-hidden', '1')
    var nodes = host.querySelectorAll ? host.querySelectorAll('[data-pw-chrome-btn]') : []
    var i
    for (i = 0; i < nodes.length; i++) {
      if (nodes[i] && nodes[i].setAttribute) nodes[i].setAttribute('data-pw-hidden', '1')
    }
    var header = chromeKitHeaderOf(host)
    if (header) stampChromeKitOff(header, 'topbar')
  }
  function chromeKitTopbarInner(create) {
    ...
    if (!inner) inner = scopedQuery('.pw-topbar-inner,.pw-shop-topbar-inner')
    if (inner) return inner
  ...
  function addChromeKitTopbar(kind, html) {
    ...
    for (i = 0; i < existing.length; i++) {
      if (chromeKitTopbarKindKey(existing[i].getAttribute('data-pw-chrome-btn') || '') === key) {
        existing[i].removeAttribute('data-pw-hidden')
        if (existing[i].style) existing[i].style.removeProperty('display')
        revealChromeKitTopbarHost()
        post('dirty', {})
        listChromeKitState()
        selectEl(existing[i])
        return
  ...
  function headerLogoOffsetHeader() {
    return document.querySelector('header.pw-header, header.pw-shop-header, .pw-shop-header, header')
  }
  function headerLogoOffsetPaintTargets() {
    var header = headerLogoOffsetHeader()
    var brand = headerLogoOffsetHost()
    var cluster = header && header.querySelector ? header.querySelector('.pw-brand-cluster, .pw-shop-brand-cluster') : null
    var main = header && header.querySelector ? header.querySelector('.pw-header-main, .pw-shop-header-inner') : null
    var frame = (brand && brand.querySelector && brand.querySelector('.pw-logo-frame, [data-pw-logo-frame="1"]'))
      || (cluster && cluster.querySelector && cluster.querySelector('.pw-logo-frame, [data-pw-logo-frame="1"]'))
      || (header && header.querySelector && header.querySelector('.pw-logo-frame, [data-pw-logo-frame="1"]'))
    var out = []
  ...
  function paintLogoOffsetCssVars(el, x, y) {
    if (!el || !el.style) return
    if (!x) el.style.removeProperty('--pw-logo-x')
    else el.style.setProperty('--pw-logo-x', x + 'px')
    if (!y) el.style.removeProperty('--pw-logo-y')
    else el.style.setProperty('--pw-logo-y', y + 'px')
  }
  function readLogoOffsetEl(el) {
  ...
  function headerLogoOffsetOf(root) {
    if (root) return readLogoOffsetEl(root)
    var targets = headerLogoOffsetPaintTargets()
    var i
    for (i = 0; i < targets.length; i++) {
      var got = readLogoOffsetEl(targets[i])
      if (got.x || got.y) return got
    }
    return { x: 0, y: 0 }
  }
  ...
  function applyHeaderLogoOffset(xRaw, yRaw) {
    var brand = headerLogoOffsetHost()
    var next = applyLogoOffsetOn(brand, xRaw, yRaw)
    var targets = headerLogoOffsetPaintTargets()
    var i
    for (i = 0; i < targets.length; i++) {
      paintLogoOffsetCssVars(targets[i], next.x, next.y)
    }
    return next
  }
  ...
  function chromeKitShiftRoot(bar) {
    if (bar === 'dock') return chromeKitDockRoot()
    if (bar === 'topbar') return chromeKitTopbarInner(false)
    return chromeKitHeadRoot()
  }
  function setChromeKitShift(bar, x) {
    if (bar === 'topbar') {
      var inner = chromeKitTopbarInner(false)
      var host = chromeKitTopbarRoot()
      var n = applyChromeKitShift(inner || host, x)
      if (inner && host && inner !== host) applyChromeKitShift(host, n)
    } else {
      applyChromeKitShift(chromeKitShiftRoot(bar), x)
    }
    post('dirty', {})
    listChromeKitState()
  }
  ...
  function chromeKitTopbarGapOf(root) {
    var n = chromeKitGapRawOf(root)
    if (!isFinite(n)) n = ${PW_TOPBAR_GAP_DEFAULT}
    n = Math.round(n)
    if (n > ${PW_KIT_GAP_MAX}) n = ${PW_KIT_GAP_MAX}
    if (n < ${PW_KIT_GAP_MIN}) n = ${PW_KIT_GAP_MIN}
    return n
  ...
  function setChromeKitGap(bar, gap) {
    var root = chromeKitShiftRoot(bar)
    if (bar === 'topbar') applyChromeKitGap(root, isFinite(parseInt(String(gap), 10)) ? gap : ${PW_TOPBAR_GAP_DEFAULT})
    else applyChromeKitGap(root, gap)
    post('dirty', {})
    listChromeKitState()
  }
  function listChromeKitState() {
    ...
    if (topInner) {
      applyChromeKitShift(topInner, chromeKitShiftOf(topInner))
      if (isFinite(chromeKitGapRawOf(topInner))) applyChromeKitGap(topInner, chromeKitTopbarGapOf(topInner))
    }
    ...
    post('chromeKitState', {
      ...
      topbarX: chromeKitShiftOf(topInner),
      topbarGap: chromeKitTopbarGapOf(topInner),
      logoX: logoOff.x,
      logoY: logoOff.y,
  ...
  function setChromeKitHidden(kind, bar, hidden) {
    ...
    for (var ti = 0; ti < targets.length; ti++) {
      ...
      if (hidden) {
        ...
        if (bar === 'topbar') revealChromeKitTopbarHost()
      }
  ...
  function isShopDockTryOnBtn(el) {
    if (!el || !el.getAttribute) return false
    if (el.getAttribute('data-pw-chrome-btn') !== 'try-on') return false
    if (isPdpDockFaceBtn(el)) return false
    if (el.closest && el.closest('.pw-pdp-sticky-nav,.pw-pdp-sticky-ctas')) return false
    return true
  }
  ...
  function revealShopDockTryOnBtn(el) {
    if (!el || !el.setAttribute) return el
    try { el.removeAttribute('data-pw-pdp-home') } catch (errPdpHomeTry) {}
    try { el.removeAttribute('data-pw-pdp-nav') } catch (errPdpNavTry) {}
    el.setAttribute('data-pw-chrome-kit', '1')
    el.setAttribute('data-pw-dock-show', 'shop')
    el.setAttribute('data-pw-dock-slot', 'icon')
    el.setAttribute('data-nanoai-try-on', '')
    if (el.style) {
      el.style.removeProperty('display')
      el.style.removeProperty('visibility')
      el.style.removeProperty('opacity')
  ...
  function dedupeShopDockTryOnBtns() {
    var dock = chromeKitDockRoot()
    if (!dock || !dock.querySelectorAll) return null
    var nodes = dock.querySelectorAll('[data-pw-chrome-btn="try-on"]')
    var kept = null
    var i
    for (i = 0; i < nodes.length; i++) {
      var n = nodes[i]
      ...
      if (isPdpDockFaceBtn(n)) continue
      if (!kept) { kept = n; continue }
      try { if (n.parentNode) n.parentNode.removeChild(n) } catch (errDupTry) {}
    }
    return kept
  }
  function shopDockTryOnDefaultHtml() {
    var loc = String((document.documentElement && document.documentElement.getAttribute('lang')) || 'vi').slice(0, 2)
    return PW_SHOP_DOCK_TRYON_HTML[loc] || PW_SHOP_DOCK_TRYON_HTML.vi || ''
  }
  function ensureShopDockTryOnBtn() {
    var dock = chromeKitDockRoot()
    if (!dock) return null
    var existing = dedupeShopDockTryOnBtns() || findChromeKitBtn('try-on', 'dock')
    if (existing) return revealShopDockTryOnBtn(existing)
    var html = shopDockTryOnDefaultHtml()
    if (!html) return null
    var wrap = document.createElement('div')
    wrap.innerHTML = html
    var el = revealShopDockTryOnBtn(wrap.firstElementChild)
    if (!el) return null
    var face = dock.querySelector('.pw-pdp-sticky-nav,.pw-pdp-sticky-ctas')
    try {
      if (face) dock.insertBefore(el, face)
      else dock.appendChild(el)
    } catch (errInsTry) { return null }
    return el
  }
  function setChromeKitDockShow(kind, show) {
    ...
    if (!el && kind === 'try-on' && String(show || '') !== 'off' && !isPdpEditorDoc()) {
      el = ensureShopDockTryOnBtn()
    }
    if (!el) return
  ...
  function removeSelectedRegionBlock() {
    ...
    if (isTopbarHost(bar) || topbarHostOf(selected) === bar) {
      hideChromeKitTopbarHost(bar)
      selected = null
      hideResizeHandle()
      hideDeleteHandle()
      hideMoveHandle()
      ...
      postHidden()
      syncLayerSwitches()
      try { listChromeKitState() } catch (errKitDel) {}
      return true
    }
  ...
  function listInsertGapUnits() {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (unit.closest && unit.closest('header, .pw-header, .pw-shop-header, [data-pw-region="header"], [data-pw-region="nav"], [data-pw-region="topbar"], .pw-topbar, .pw-nav-main, .pw-bottom-nav, [data-pw-chrome-kit="dock"], [data-pw-chrome-kit="float"]')) continue
  ...
  function ensureHeaderLogoSlot() {
    ...
    if (isImgEl(existing)) {
      ...
      if (existFrame && !isFilledLogo(existing) && existFrame.getAttribute('data-pw-logo-user-size') !== '1') {
        ...
        var fallback = defaultLogoFrameBox()
        if (!(existBox.w > 24 && existBox.h > 16) || (isMobileEdit() && existBox.w > fallback.w + 8)) {
          var existSize = clampLogoBox(fallback.w, fallback.h, true)
    ...
    img.setAttribute('alt', 'logo')
    img.setAttribute('src', 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7')
    var slotSize = clampLogoBox(defaultLogoFrameBox().w, defaultLogoFrameBox().h, true)
    img.style.width = slotSize.w + 'px'
    img.style.height = slotSize.h + 'px'
    img.style.objectFit = 'contain'
    img.style.backgroundColor = 'transparent'
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      '.pw-rec-fav.is-active svg,.pw-rec-fav[aria-pressed="true"] svg{fill:#fff;stroke:#fff}',
      'html[data-pw-edit-device="mobile"] .pw-pdp-sticky-nav .is-fav svg,html[data-pw-scene-lock="mobile"] .pw-pdp-sticky-nav .is-fav svg{color:#fff;fill:none;stroke:currentColor}',
      'html[data-pw-edit-device="mobile"] .pw-pdp-sticky-nav .is-fav[aria-pressed="true"] svg,html[data-pw-scene-lock="mobile"] .pw-pdp-sticky-nav .is-fav[aria-pressed="true"] svg{color:var(--pw-primary);fill:currentColor;stroke:currentColor}',
      ...
      'html[data-pw-edit-device="tablet"] .nanoai-ve-active .pw-logo-frame:has([data-pw-logo-empty="1"]),html[data-pw-edit-device="tablet"] .nanoai-ve-active [data-pw-logo-frame="1"]:has([data-pw-logo-empty="1"]),html[data-pw-edit-device="tablet"] .nanoai-ve-active img[data-pw-logo-empty="1"]{min-width:72px!important;min-height:28px!important}',
      'html[data-pw-edit-device="mobile"] .nanoai-ve-active .pw-logo-frame:has([data-pw-logo-empty="1"]),html[data-pw-edit-device="mobile"] .nanoai-ve-active [data-pw-logo-frame="1"]:has([data-pw-logo-empty="1"]),html[data-pw-edit-device="mobile"] .nanoai-ve-active img[data-pw-logo-empty="1"]{min-width:64px!important;min-height:28px!important}',
      ...
      '[data-pw-edit-device="mobile"] .pw-header-main,[data-pw-edit-device="mobile"] .pw-shop-header-inner,.nanoai-ve-mobile .pw-header-main,.nanoai-ve-mobile .pw-shop-header-inner{overflow:visible!important;min-width:0!important;width:100%!important;max-width:none!important;margin-left:0!important;margin-right:0!important;padding:8px 10px!important;padding-left:8px!important;padding-right:10px!important}',
      '[data-pw-edit-device="tablet"] .pw-header-main,[data-pw-edit-device="tablet"] .pw-shop-header-inner,.nanoai-ve-tablet .pw-header-main,.nanoai-ve-tablet .pw-shop-header-inner{overflow:visible!important;min-width:0!important;width:100%!important;max-width:none!important;margin-left:0!important;margin-right:0!important;padding:10px 16px!important}',
      '.nanoai-ve-active .pw-header-main,.nanoai-ve-active .pw-shop-header-inner{overflow:visible!important;min-width:0!important;display:flex!important;align-items:center!important}',
      'html:not([data-pw-edit-device="mobile"]):not([data-pw-scene-lock="mobile"]) .nanoai-ve-active .pw-header-main,html:not([data-pw-edit-device="mobile"]):not([data-pw-scene-lock="mobile"]) .nanoai-ve-active .pw-shop-header-inner{flex-wrap:nowrap!important}',
      '[data-pw-edit-device="tablet"] .pw-brand-cluster,[data-pw-edit-device="tablet"] .pw-shop-brand-cluster,.nanoai-ve-tablet .pw-brand-cluster,.nanoai-ve-tablet .pw-shop-brand-cluster{max-width:none!important;overflow:visible!important;flex:0 0 auto!important}',
      ...
      '[data-pw-edit-device="tablet"] .pw-header-search,[data-pw-edit-device="tablet"] .pw-shop-search-wrap,.nanoai-ve-tablet .pw-header-search,.nanoai-ve-tablet .pw-shop-search-wrap{flex:1 1 0%!important;min-width:180px!important;min-height:36px!important;width:auto!important;max-width:100%!important;margin:0!important;transform:none!important;left:auto!important;top:auto!important;right:auto!important;bottom:auto!important;position:relative!important;opacity:1!important;visibility:visible!important}',
      ...
      '@media (max-width:899px){${PW_SCENE_UNLOCKED_HTML} .pw-brand-cluster,${PW_SCENE_UNLOCKED_HTML} .pw-shop-brand-cluster{max-width:200px!important;overflow:visible!important}${PW_SCENE_UNLOCKED_HTML} .pw-header-search,${PW_SCENE_UNLOCKED_HTML} .pw-shop-search-wrap{flex:1 1 0%!important;min-width:96px!important;min-height:36px!important;width:auto!important;max-width:100%!important;transform:none!important;position:relative!important;left:auto!important;top:auto!important;opacity:1!important;visibility:visible!important}}',
      ...
      '@media (max-width:899px){${PW_SCENE_UNLOCKED_HTML} .nanoai-ve-active .pw-header-main,${PW_SCENE_UNLOCKED_HTML} .nanoai-ve-active .pw-shop-header-inner{display:flex!important;flex-wrap:nowrap!important;align-items:center!important;column-gap:6px!important;overflow:visible!important;min-width:0!important;max-width:100%!important;padding:8px 10px!important}${PW_SCENE_UNLOCKED_HTML} .pw-brand-cluster,${PW_SCENE_UNLOCKED_HTML} .pw-shop-brand-cluster{flex:0 0 auto!important;width:auto!important;max-width:200px!important;overflow:visible!important}${PW_SCENE_UNLOCKED_HTML} .pw-header a.pw-brand:not([data-pw-logo-float]),${PW_SCENE_UNLOCKED_HTML} .pw-shop-header a.pw-shop-brand:not([data-pw-logo-float]),${PW_SCENE_UNLOCKED_HTML} .pw-header a[data-pw-logo-home]:not([data-pw-logo-float]),${PW_SCENE_UNLOCKED_HTML} .pw-shop-header a[data-pw-logo-home]:not([data-pw-logo-float]){max-width:none!important}${PW_SCENE_UNLOCKED_HTML} .pw-header-search,${PW_SCENE_UNLOCKED_HTML} .pw-shop-search-wrap{flex:1 1 0%!important;min-width:96px!important;min-height:36px!important;width:auto!important;max-width:100%!important;margin:0!important;transform:none!important;position:relative!important;left:auto!important;top:auto!important;opacity:1!important;visibility:visible!important}${PW_SCENE_UNLOCKED_HTML} .pw-header-actions,${PW_SCENE_UNLOCKED_HTML} .pw-shop-header-actions{flex:0 0 auto!important;display:flex!important;flex-wrap:nowrap!important;align-items:center!important;width:auto!important;max-width:none!important;margin-left:auto!important}}',
      ...
      ${JSON.stringify(PW_CHROME_ICON_SQUARE_CSS)},
      '.pw-pdp-sticky-nav .is-try,.pw-pdp-sticky-nav .is-fav,.pw-pdp-sticky-nav [data-pw-chrome-btn="try-on"],.pw-pdp-sticky-nav [data-pw-chrome-btn="favorite-product"]{border:none!important;outline:none!important;box-shadow:none!important;background:transparent!important;-webkit-appearance:none!important;appearance:none!important;aspect-ratio:auto!important}',
      '.pw-pdp-sticky-nav .is-try .pw-chrome-icon-wrap,.pw-pdp-sticky-nav .is-fav .pw-chrome-icon-wrap,.pw-pdp-sticky-nav [data-pw-chrome-btn="try-on"] .pw-chrome-icon-wrap,.pw-pdp-sticky-nav [data-pw-chrome-btn="favorite-product"] .pw-chrome-icon-wrap{border:none!important;outline:none!important;box-shadow:none!important;background:transparent!important;border-radius:0!important}',
      ...
      'html[data-pw-edit-device="mobile"] .pw-brand-cluster,html[data-pw-edit-device="mobile"] .pw-shop-brand-cluster,html[data-pw-edit-device="tablet"] .pw-brand-cluster,html[data-pw-edit-device="tablet"] .pw-shop-brand-cluster{pointer-events:auto!important}',
      'html[data-pw-edit-device="mobile"] .pw-cat-btn,html[data-pw-edit-device="mobile"] .pw-shop-cat-btn,html[data-pw-edit-device="mobile"] [data-pw-el="cat-toggle"],html[data-pw-edit-device="mobile"] [data-pw-cat-toggle],html[data-pw-edit-device="mobile"] .pw-chrome-cat-wrap,html[data-pw-edit-device="mobile"] [data-pw-chrome-btn="back"],html[data-pw-edit-device="mobile"] [data-pw-head-back],html[data-pw-edit-device="tablet"] .pw-cat-btn,html[data-pw-edit-device="tablet"] .pw-shop-cat-btn,html[data-pw-edit-device="tablet"] [data-pw-el="cat-toggle"],html[data-pw-edit-device="tablet"] [data-pw-cat-toggle],html[data-pw-edit-device="tablet"] .pw-chrome-cat-wrap,html[data-pw-edit-device="tablet"] [data-pw-chrome-btn="back"],html[data-pw-edit-device="tablet"] [data-pw-head-back]{position:relative!important;z-index:200!important;pointer-events:auto!important}'
    ].join('')
  ...
  function activate() {
    ...
    try { scheduleEditorChromeFit() } catch (errChromeFit) {}
    ...
    try { if (!isPdpEditorDoc()) { dedupeShopDockHomeBtns(); ensureShopDockHomeBtn(); dedupeShopDockTryOnBtns(); ensureShopDockTryOnBtn() } } catch (errShopHome) {}
  ...
  function pdpDockKeepScore(el) {
    ...
    if (isShopDockTryOnBtn(el) || isShopDockHomeBtn(el)) s -= 20
    return s
  }
  function restorePdpBuyBoxActionsFromDock() {
    ...
    for (k = 0; k < kinds.length; k++) {
      ...
      for (i = 0; i < nodes.length; i++) {
        var el = nodes[i]
        if (el === keep) continue
        if (isShopDockHomeBtn(el) || isShopDockTryOnBtn(el)) continue
