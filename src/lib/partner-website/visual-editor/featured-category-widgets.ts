...
function placeholderTiles(count: number, locale: WebLocale): string {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    out += `<a class="pw-featured-cat-card" ${pwElAttr(PW_EL.card)} data-pw-grid-placeholder="1" href="#">
  <span class="pw-featured-cat-media" ${pwElAttr(PW_EL.cardMedia)}></span>
  <span ${pwElAttr(PW_EL.cardName)}>${escapeHtml(name)}</span>
</a>`
  }
  return out
}
...
export function buildVisualEditorFeaturedCategoriesHtml(input: {
  ...
  const limit = Math.max(pageSize, Math.min(20, Math.floor(Number(input.limit) || pageSize || 10)))
  ...
  return `<section class="pw-featured-cat" ${pwRegionAttr(PW_REGION.categories)}${pwKindSceneAttr(PW_KIND_SCENE_MEDIA)} data-pw-featured-categories="1" data-pw-grid-kind="featured-categories" data-pw-grid-cols="5" data-pw-grid-cols-mobile="2" data-pw-grid-rows="${rows}" data-limit="${limit}" style="margin:0;padding:0;min-height:0;height:auto">
  <div class="pw-featured-cat-inner">
    <div data-pw-grid class="pw-featured-cat-grid" ${pwElAttr(PW_EL.grid)}>${placeholderTiles(pageSize, locale)}</div>
