...
export const PARTNER_WEBSITE_PAGE_CATALOG: PartnerWebsitePageDef[] = [
  ...
  },
  {
    ...
    discoveryKeys: PAGE_FEATURE_KEYS,
    defaultFeaturesVi: [
      'Một vỏ listing /c/{path}: head giống trang chủ, bộ lọc, lưới SP',
      'Sửa nhanh một trang mẫu — live bind tên danh mục đang mở',
    ],
    defaultFeaturesEn: [
      'One listing shell for every /c/{path}: homepage chrome, filters, product grid',
      'Edit once in Sửa nhanh — live binds the open category name',
    ],
  },
  {
    key: 'lookbook',
...
> {
  if (locale === 'en') {
    return {
      ...
      collection: { title: 'Category page', hint: 'One listing shell for every /c/{path}' },
  ...
  if (locale === 'zh') {
    return {
      ...
      collection: { title: '分类页', hint: '一个模板用于所有 /c/{path}' },
  ...
  if (locale === 'ja') {
    return {
      ...
      collection: { title: 'カテゴリページ', hint: 'すべての /c/{path} で同じシェル' },
  ...
  if (locale === 'ko') {
    return {
      ...
      collection: { title: '카테고리 페이지', hint: '모든 /c/{path}에 같은 셸' },
  ...
  return {
    ...
    collection: { title: 'Trang danh mục', hint: 'Một vỏ listing cho mọi /c/{path}' },
