...
test('chrome layout css is injected once before </head>', () => {
  ...
  assert.equal(once.includes('[data-pw-promo-carousel]'), true)
  assert.equal(once.includes('[data-pw-featured-categories]:not([data-pw-featured-live])'), true)
  assert.equal(once.includes('body:not(.nanoai-ve-active) [data-pw-grid-placeholder]'), true)
  assert.equal(once.includes('[data-pw-sample-preview]'), true)
  ...
  assert.equal(once.includes('#pw-tap-ack-bar'), true)
  assert.equal(once.includes('[data-pw-tapped="1"]'), true)
  assert.equal(once.includes('Đã nhận thao tác'), false)
  ...
  assert.equal(once.includes('html[data-pw-page="home"] .pw-bottom-nav[data-pw-chrome-kit="dock"]:not(:has([aria-current="page"])) > [data-pw-chrome-btn="home"]'), true)
  assert.equal(once.includes('id="pw-shop-dock-nav-active"'), true)
  assert.equal(once.includes("setAttribute(ATTR,'page')"), true)
  ...
  assert.match(once, /\.pw-pdp-sticky-nav \.is-try,.pw-pdp-sticky-nav \.is-fav/)
  assert.match(once, /appearance:none!important/)
  ...
  assert.equal(once.includes('html .pw-footer,html .pw-shop-footer{color:#111827!important}'), true)
  assert.equal(once.includes('.pw-footer .pw-logo-frame'), true)
  assert.equal(once.includes('--pw-logo-box-h,40px'), true)
  assert.equal(once.includes('html .pw-footer-bar a.pw-shop-footer-moit[style*="--pw-btn-color"]'), true)
  ...
  assert.equal(once.includes('[data-pw-inline-visual-root][data-pw-active-device="desktop"] .pw-shop-footer-inner'), true)
  assert.equal(once.includes('grid-template-columns:minmax(0,1.25fr) repeat(4,minmax(0,1fr))!important'), true)
  assert.equal(once.includes('html .pw-shop-footer-brand,html .pw-footer-brand,html .pw-footer-col,html .pw-shop-footer-col{grid-column:auto!important;min-width:0!important;max-width:100%!important}'), true)
  assert.match(once, /html \.pw-newsletter[\s\S]*flex-direction:column!important/)
  assert.match(once, /html \.pw-newsletter input[\s\S]*min-width:0!important/)
  ...
  assert.equal(once.includes('[data-pw-region="cart-list"]'), true)
  assert.equal(once.includes('[data-pw-region="account-main"]'), true)
  assert.equal(once.includes('.pw-shop-info img'), true)
  ...
  assert.equal(once.includes('html[data-pw-edit-device="mobile"],html[data-pw-scene-lock="mobile"]{--pw-page-gutter:4px;--pw-block-w:min(calc(var(--pw-scene-w,100%) - 8px),var(--pw-content,1200px));--pw-chrome-inset:0px}'), true)
  ...
  assert.equal(once.includes('html[data-pw-scene-zoomed="1"] [data-pw-live-chrome-scale]{transform:scale'), false)
  assert.equal(once.includes('[data-pw-live-chrome-scale]{width:100%!important'), true)
  ...
  assert.equal(once.includes('function uaDevice(){'), true)
  ...
  assert.equal(once.includes('html[data-pw-pdp-desktop-sticky="1"][data-pw-page="product"] .pw-bottom-nav[data-pw-chrome-kit="dock"]'), true)
  assert.equal(once.includes('html[data-pw-pdp-desktop-sticky="1"] [data-pw-live-dock]>.pw-bottom-nav[data-pw-chrome-kit="dock"]'), true)
  assert.equal(once.includes('position:relative!important;left:auto!important'), true)
  assert.equal(once.includes('max-width:33rem!important'), true)
  assert.equal(once.includes('min-width:160px!important'), true)
  assert.equal(once.includes('padding:8px max(16px,calc((100% - 33rem) / 2))!important'), true)
  assert.equal(once.includes('min-height:48px!important'), true)
  assert.equal(once.includes('padding:12px 20px!important'), true)
  ...
  assert.equal(once.includes('html[data-pw-edit-device="tablet"] .pw-product-grid'), true)
  assert.equal(once.includes('grid-template-columns:repeat(3,minmax(0,1fr))!important'), true)
  ...
  assert.equal(once.includes('--pw-page-gutter:16px'), true)
  assert.equal(once.includes('--pw-page-gutter:24px'), true)
  ...
  assert.equal(once.includes('top:50%!important'), true)
  assert.equal(once.includes('transform:translate(calc(-100% + var(--pw-logo-x, 0px)), calc(-50% + var(--pw-logo-y, 0px)))!important'), true)
  ...
  assert.equal(once.includes('.pw-logo,.pw-shop-logo{height:28px!important}'), false)
  assert.equal(once.includes('width:var(--pw-logo-box-w,140px)!important'), true)
  assert.equal(once.includes('height:var(--pw-logo-box-h,36px)!important'), true)
  ...
  assert.equal(once.includes('[data-pw-chrome-btn="cart"]{order:-1'), false)
  assert.equal(once.includes('html[data-pw-edit-device="mobile"] .pw-header-actions > *'), true)
  assert.equal(once.includes('.pw-shop-header-actions > *{order:0!important}'), true)
  assert.equal(once.includes('pw-shop-mobile-header-logo'), true)
  assert.equal(once.includes('pw-shop-mobile-head-back'), true)
  assert.equal(once.includes('[data-pw-chrome-btn="back"]'), true)
  assert.equal(once.includes('history.back'), true)
  assert.equal(once.includes('nanoai-ve-active'), true)
  assert.equal(once.includes('pw-shop-listing-head'), true)
  assert.equal(once.includes('data-pw-listing-filter-slot'), true)
  assert.equal(once.includes('data-pw-head-compact'), true)
  ...
  assert.equal(once.includes('.pw-nav-main .pw-nav-pill>a:hover'), true)
  assert.equal(once.includes('.pw-nav-main .pw-nav-row-scroll>a:hover'), true)
  assert.equal(
    once.includes(
      'html .pw-nav-main .pw-nav-row-scroll,html .pw-shop-nav-row .pw-nav-row-scroll{display:flex!important;flex-wrap:nowrap!important;justify-content:flex-start!important'
    ),
    true
  )
  assert.equal(once.includes('html .pw-topbar[data-pw-hidden="1"]'), true)
  assert.equal(once.includes('html [data-pw-chrome-kit="topbar"][data-pw-hidden="1"]'), true)
  assert.equal(once.includes('.pw-nav-flyout-bar a:hover'), true)
  assert.equal(once.includes('data-pw-search-history-on'), false)
  assert.equal(once.includes('[data-pw-search-history]'), true)
  assert.equal(once.includes('.pw-shop-search-compose'), true)
  assert.equal(once.includes('.pw-search-compose'), true)
  assert.equal(once.includes('.pw-shop-main-compose{padding:0!important;margin:0!important;min-height:100dvh;overflow:visible'), true)
...
test('chrome layout paints marketplace desktop header with the primary token', () => {
  ...
  assert.match(out, /\[data-pw-live-chrome\]\[data-pw-look="marketplace"\] \.pw-header/)
  assert.equal(
