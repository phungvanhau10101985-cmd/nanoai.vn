...
test('listing head attaches filters and collapses after scroll', () => {
  ...
  assert.match(PW_LISTING_HEAD_CSS, /html\[data-pw-head-compact="1"\] \.pw-nav-main/)
  assert.match(PW_LISTING_HEAD_CSS, /html\[data-pw-page\]\[data-pw-head-compact="1"\] \.pw-nav-main/)
  assert.match(PW_LISTING_HEAD_CSS, /html\[data-pw-head-compact="1"\]:not\(\[data-pw-edit-device\]\):not\(\[data-pw-scene-lock\]\) \.pw-nav-main/)
  assert.equal(PW_LISTING_HEAD_CSS.includes('.pw-header-search{display:none'), false)
  ...
  assert.match(PARTNER_SHOP_LISTING_HEAD_SCRIPT, /data-pw-live-chrome-scale/)
  assert.match(PARTNER_SHOP_LISTING_HEAD_SCRIPT, /data-pw-visual-ready/)
  assert.match(PARTNER_SHOP_LISTING_HEAD_SCRIPT, /__pwListingHeadSync/)
  assert.match(PW_LISTING_HEAD_CSS, /data-pw-listing-filters-ready/)
  assert.match(PW_LISTING_HEAD_CSS, /\[data-pw-live-chrome\] \[data-pw-listing-filter-slot\]/)
  assert.match(PARTNER_SHOP_LISTING_HEAD_SCRIPT, /nanoai-ve-active/)
  assert.match(PARTNER_SHOP_LISTING_HEAD_SCRIPT, /canCompact/)
  assert.match(PARTNER_SHOP_LISTING_HEAD_SCRIPT, /shop\.setAttribute\(ATTR,'1'\)/)
  assert.equal(PARTNER_SHOP_LISTING_HEAD_SCRIPT.includes('!isListing()||editor()'), false)
...
test('chrome layout and shop theme ship listing head CSS and script', () => {
  ...
  assert.match(html, /html\[data-pw-head-compact="1"\] \.pw-nav-main/)
  assert.match(html, /html\[data-pw-page\]\[data-pw-head-compact="1"\] \.pw-topbar/)
})
