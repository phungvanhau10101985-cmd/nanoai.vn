...
import { shopAboveFoldBannerSrc, shopBannerDisplaySrc } from '@/lib/partner-website/shop/inventory-shop-detail'
  ...
  const slides = items
    .map((item, i) => {
      ...
      const src = escapeAttr(
        (i === 0 ? shopAboveFoldBannerSrc(item.image_url) : shopBannerDisplaySrc(item.image_url)) ||
          item.image_url
      )
