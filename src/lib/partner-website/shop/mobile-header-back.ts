/**
 * Compact head back — UX 188: previous page, not Home.
 * Engine for every shop. Stamp on mobile + tablet HTML; CSS hides on home.
 */
...
export function isPartnerShopHeadBackFace(input: {
  editDevice?: string | null
  sceneLock?: string | null
  queryDevice?: string | null
  viewportCompact?: boolean
}): boolean {
  const stamped = String(input.queryDevice || input.editDevice || input.sceneLock || '')
    .trim()
    .toLowerCase()
  if (stamped === 'mobile' || stamped === 'tablet') return true
  if (stamped === 'desktop' || stamped === 'laptop') return false
  return Boolean(input.viewportCompact)
}
...
export function isMobileHeadBackTarget(html: string, device?: VisualDeviceVariant | null): boolean {
  ...
  return (
    /\bdata-pw-edit-device=["'](mobile|tablet)["']/i.test(html) ||
    /\bdata-pw-scene-lock=["'](mobile|tablet)["']/i.test(html)
  )
}
...
/** HTML cũ thiếu nút — gắn trước Danh mục trong header. File máy mobile + tablet. */
...
const BACK_FACE =
  'display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;width:40px!important;height:40px!important;min-width:40px!important;min-height:40px!important;padding:0!important;margin:0 6px 0 0!important;gap:0!important;border:1.5px solid rgba(255,255,255,.55)!important;background:rgba(255,255,255,.16)!important;color:#fff!important;border-radius:10px!important;box-shadow:none!important;cursor:pointer!important;position:relative!important;z-index:200!important;pointer-events:auto!important'

const BACK_FACE_PHONE = `${BACK_FACE};order:1!important`
/** Tablet cluster là flex một hàng — order:0 để nút đứng trước Danh mục theo DOM. */
const BACK_FACE_TABLET = `${BACK_FACE};order:0!important`

function mobileHeadBackShowBlock(host: string, face: string = BACK_FACE_PHONE): string {
  return (
    `${host} [data-pw-chrome-btn="back"]:not([data-pw-hidden="1"]),${host} [data-pw-head-back]:not([data-pw-hidden="1"]){${face}}` +
...
const TABLET_NOT_HOME = [
  'html[data-pw-edit-device="tablet"]:not([data-pw-page="home"]):not(:has([data-pw-page="home"]))',
  'html[data-pw-scene-lock="tablet"]:not([data-pw-page="home"]):not(:has([data-pw-page="home"]))',
]

const UNLOCKED_NOT_HOME =
  'html:not([data-pw-edit-device]):not([data-pw-scene-lock]):not([data-pw-page="home"]):not(:has([data-pw-page="home"]))'

/** Mặc định ẩn. Hiện trên mobile + tablet khi trang không phải home. Không phải nút Home. */
