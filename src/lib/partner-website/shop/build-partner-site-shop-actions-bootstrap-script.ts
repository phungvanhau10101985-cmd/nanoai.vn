...
var HANDOFF_API=${JSON.stringify(`/api/site/${encodeURIComponent(slug)}/auth/handoff`)};
var PENDING_CART_KEY=${JSON.stringify(`pw_pending_cart_v1:${slug.trim().toLowerCase()}`)};
...
function readPendingCart(){
  try{
    var raw=sessionStorage.getItem(PENDING_CART_KEY);
    var parsed=raw?JSON.parse(raw):[];
    return Array.isArray(parsed)?parsed:[];
  }catch(e){return [];}
}
function writePendingCart(list){
  try{sessionStorage.setItem(PENDING_CART_KEY,JSON.stringify(list||[]));}catch(e){}
}
function queuePendingCart(product,opts){
  opts=opts||{};
  var id=String(product&&product.inventory_id||'').trim();
  ...
  var next={
    ...
    price_hint:String(product.price_hint||'').trim(),
    sku:String(product.sku||'').trim(),
    color:String(product.color||'').trim(),
    size:String(product.size||'').trim(),
    quantity:Math.min(99,Math.max(1,Math.round(Number(product.quantity)||1))),
    buyNow:!!opts.buyNow
  };
  var list=readPendingCart();
  var found=false;
  for(var i=0;i<list.length;i++){
    var it=list[i]||{};
    if(String(it.inventory_id||'')===next.inventory_id&&String(it.color||'')===next.color&&String(it.size||'')===next.size){
      it.quantity=Math.min(99,(Number(it.quantity)||1)+next.quantity);
      if(next.buyNow)it.buyNow=true;
      found=true;
      break;
    }
  }
  if(!found)list.push(next);
  writePendingCart(list);
}
function takePendingCart(){
  var list=readPendingCart();
  try{sessionStorage.removeItem(PENDING_CART_KEY);}catch(e){}
  return list;
}
function consumeGoogleHandoff(){
  if(window.__pwAuthHandoffDone)return Promise.resolve(false);
  var sp=new URLSearchParams(location.search);
  var token=(sp.get('pw_auth')||'').trim();
  if(!token)return Promise.resolve(false);
  window.__pwAuthHandoffDone=true;
  return apiFetch(HANDOFF_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:token})}).then(function(res){
    var aid=res.j&&res.j.accountId;
    if(aid)persistAccount(aid);
    sp.delete('pw_auth');
    sp.delete('meta_complete_registration');
    var nextPath=location.pathname+(sp.toString()?'?'+sp.toString():'')+(location.hash||'');
    try{history.replaceState(null,'',nextPath);}catch(e){}
    return !!(res.ok&&res.j&&res.j.ok);
  }).catch(function(){window.__pwAuthHandoffDone=false;return false;});
}
...
function apiFetch(url,opts){
  ...
  return fetch(url,opts).then(function(r){
    captureSession(r);
    var aid=r.headers.get(ACCOUNT_HDR);
    if(aid)persistAccount(aid);
    return r.json().then(function(j){return {ok:r.ok,j:j,status:r.status};}).catch(function(){return {ok:r.ok,j:{},status:r.status};});
  });
}
function toast(msg){
...
function readProductFromEl(el){
  ...
  if(!id||!UUID_RE.test(id)){
    var pageHost=document.querySelector('[data-pw-region="pdp-info"][data-inventory-id],.pw-pdp[data-inventory-id],body[data-inventory-id],html[data-inventory-id],[data-pw-page="product"][data-inventory-id],[data-pw-live-dock] [data-inventory-id],[data-nanoai-inventory]');
    if(pageHost){
...
function addToCart(product, opts){
  opts=opts||{};
  if(!accountId()){
    if(opts.skipLogin)return Promise.resolve(false);
    queuePendingCart(product,{buyNow:!!(opts.buyNow||opts.silent)});
...
function flushPendingCart(){
  if(window.__pwPendingCartFlushInFlight)return Promise.resolve();
  if(!accountId())return Promise.resolve();
  var path=(location.pathname||'').replace(/\\/+$/,'')||'/';
  if(path==='/login'||path==='/site/'+SITE_SLUG+'/login')return Promise.resolve();
  var list=readPendingCart();
  if(!list.length)return Promise.resolve();
  window.__pwPendingCartFlushInFlight=true;
  list=takePendingCart();
  if(!list.length){
    window.__pwPendingCartFlushInFlight=false;
  ...
  for(var i=0;i<list.length;i++){
    (function(item){
      if(item&&item.buyNow)buy=true;
      seq=seq.then(function(ok){
        return addToCart(item,{silent:true,skipLogin:true}).then(function(next){return ok&&next;});
      });
    })(list[i]);
  }
  return seq.then(function(ok){
    if(!ok){
      window.__pwPendingCartFlushInFlight=false;
      for(var j=0;j<list.length;j++)queuePendingCart(list[j],{buyNow:!!(list[j]&&list[j].buyNow)});
      return;
    }
...
function favoriteToggleBusy(){
  var m=window.__pwFavoriteToggleInFlight;
  if(!m)return false;
  for(var k in m){if(Object.prototype.hasOwnProperty.call(m,k))return true;}
  return false;
}
function bumpFavoriteFetchGen(){
  window.__pwFavoriteFetchGen=(window.__pwFavoriteFetchGen||0)+1;
  return window.__pwFavoriteFetchGen;
}
function patchFavoriteCache(id,on){
  window.__pwFavoriteIdsCache=window.__pwFavoriteIdsCache||{};
  if(on)window.__pwFavoriteIdsCache[id]=1;
  else delete window.__pwFavoriteIdsCache[id];
}
function applyFavoriteToProduct(id,on,likes){
  var all=document.querySelectorAll('[data-pw-favorite],[data-pw-pdp-favorite],[data-pw-chrome-btn="favorite-product"]');
...
function toggleFavorite(product,btn){
  ...
  var optimistic=Math.max(0,base+(next?1:-1));
  bumpFavoriteFetchGen();
  applyFavoriteToProduct(id,next,optimistic);
  if(btn&&!readProductFromEl(btn))applyFavoriteState(btn,next,optimistic);
  var req=apiFetch(EVENTS_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'toggle_favorite',inventory_id:product.inventory_id})}).then(function(res){
    if(!res.ok){
      applyFavoriteToProduct(id,was,base);
      if(btn&&!readProductFromEl(btn))applyFavoriteState(btn,was,base);
      toast(COPY.error);
      return;
    }
    var on=!!(res.j&&res.j.is_favorite);
    var likes=res.j&&typeof res.j.likes_count==='number'?Math.max(0,Math.round(res.j.likes_count)):optimistic;
    applyFavoriteToProduct(id,on,likes);
    if(btn&&!readProductFromEl(btn))applyFavoriteState(btn,on,likes);
    hydrateChromeBadges(true);
  }).finally(function(){
  ...
  apiFetch(FAV_API).then(function(res){
    ...
    if(rawIds.length){
      for(var i=0;i<rawIds.length;i++){
        var fid=String(rawIds[i]||'').toLowerCase();
        if(fid)ids[fid]=1;
      }
    }else{
    ...
    var cache=window.__pwFavoriteIdsCache||{};
    var inflight=window.__pwFavoriteToggleInFlight||{};
    for(var k in inflight){
      if(!Object.prototype.hasOwnProperty.call(inflight,k))continue;
      if(cache[k])ids[k]=1;
      else delete ids[k];
    }
    if(gen!==window.__pwFavoriteFetchGen&&!favoriteToggleBusy()){
      /* stale empty fetch after a completed toggle: keep optimistic ids */
      for(var ck in cache){
        if(Object.prototype.hasOwnProperty.call(cache,ck)&&cache[ck])ids[ck]=1;
      }
    }
...
if(document.documentElement.getAttribute('data-pw-shop-actions-bound')!=='1'){
  ...
  document.addEventListener('click',function(ev){
    ...
    var favBtn=t.closest('[data-pw-favorite],[data-pw-pdp-favorite],[data-pw-chrome-btn="favorite-product"]');
    if(!favBtn){
    ...
    if(!favBtn){
      try{
        ...
        for(var fi=0;fi<stack.length;fi++){
          ...
          var hitFav=node.closest('[data-pw-favorite],[data-pw-pdp-favorite],[data-pw-chrome-btn="favorite-product"],.pw-rec-fav,.is-fav');
          if(!hitFav)continue;
          if(hitFav.getAttribute&&hitFav.getAttribute('data-pw-react-fav')==='1'){
            ev.preventDefault();ev.stopPropagation();
            if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();
            try{hitFav.click();}catch(_){}
            return;
          }
          favBtn=hitFav;
          break;
