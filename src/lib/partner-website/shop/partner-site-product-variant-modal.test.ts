...
test('variant modal CSS matches 188 layout and theme tokens', () => {
  ...
  assert.match(PW_PRODUCT_VARIANT_MODAL_CSS, /data-pw-variant-sale/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_CSS, /data-pw-variant-scroll/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_CSS, /\[data-pw-variant-card\]\{[^}]*overflow:hidden/)
  assert.doesNotMatch(PW_PRODUCT_VARIANT_MODAL_CSS, /\[data-pw-variant-foot\]\{[^}]*position:sticky/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_CSS, /@media \(hover:hover\) and \(pointer:fine\)/)
...
test('shop-actions injects PDP variant modal before add-to-cart', () => {
  ...
  assert.match(PW_PRODUCT_VARIANT_MODAL_RUNTIME_JS, /data-pw-variant-scroll/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_RUNTIME_JS, /function bindVariantConfirm/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_RUNTIME_JS, /pointerup/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_RUNTIME_JS, /function variantImg/)
  assert.match(PW_PRODUCT_VARIANT_MODAL_RUNTIME_JS, /\?view=buy/)
...
test('variant modal applies empty API size/color arrays instead of leftover DOM pills', () => {
  assert.match(PW_PRODUCT_VARIANT_MODAL_RUNTIME_JS, /if\(Array\.isArray\(p\.sizes\)\)st\.sizes=/)
