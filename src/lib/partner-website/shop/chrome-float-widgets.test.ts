...
test('chat Zalo Facebook and top-up are viewport-fixed chrome', () => {
  ...
  assert.equal(PARTNER_SHOP_CHROME_FLOAT_POS_JS.includes('function pwBindChatLogoFallback'), true)
  assert.equal(PARTNER_SHOP_CHROME_FLOAT_POS_JS.includes('data-pw-chat-logo-fallback'), true)
  assert.equal(PARTNER_SHOP_CHROME_FLOAT_SCRIPT.includes('pwBindChatLogoFallback'), true)
