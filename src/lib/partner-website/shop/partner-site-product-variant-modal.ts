...
[data-pw-variant-total-price]{font:700 18px/1.2 system-ui,sans-serif;color:var(--pw-buy)}
...
function variantQtyWide(st){
  ...
  return '<div data-pw-variant-section><p data-pw-variant-label>'+variantEsc(COPY.variantQty||COPY.quantity||'Qty')+'</p>'
    ...
    +'</div>'
    +'<div data-pw-variant-total><span data-pw-variant-total-label>'+variantEsc(String(COPY.variantLineTotal||'').replace('{n}',String(st.qty)))+'</span><span data-pw-variant-total-price>'+variantEsc(st.lineLabel)+'</span></div>'
    +'</div>';
}
...
function paintVariantModal(){
  ...
  var price=unit>0?variantMoney(unit,st.priceHint):st.priceHint;
...
function openPdpVariantModal(seed,action){
  ...
  Promise.all([
    ...
    if(p){
      ...
      st.unitPrice=Number(p.salePriceAmount||p.priceAmount||0)||0;
...
function isPdpCartTrigger(el){
  ...
  if(el.closest('article,.pw-product-card,.pw-shop-card,[data-pw-region="catalog"],[data-pw-catalog]'))return false;
  if(el.closest('[data-pw-pdp-add-cart],[data-pw-pdp-buy-now],[data-pw-pdp-bottom],[data-pw-region="pdp-info"],.pw-pdp,.pw-pdp-sticky'))return true;
  return document.body.getAttribute('data-pw-page')==='product';
}
`
