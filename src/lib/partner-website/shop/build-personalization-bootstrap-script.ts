...
function apiFetch(path,opts){
  ...
  var url=API+path;
  if(method==='GET'&&(path.indexOf('/featured-categories')===0||path==='/profile'||path.indexOf('/profile?')===0||path==='/flash-sale')){
...
function saleBadgeHtml(sale,badge,p){
  ...
  if(sale&&sale.badge){
    ...
    chip=sale.countdown&&sale.promoKind!=='clearance'?'<span class="pw-sale-chip pw-sale-chip-'+chipKind+'" data-pw-sale-countdown="'+String(sale.countdown).replace(/"/g,'')+'" data-pw-sale-phase="'+sale.kind+'" data-pw-sale-kind="'+(sale.promoKind||'')+'" data-pw-sale-label="'+(sale.program||'').replace(/"/g,'')+'">'+chipLabel+' <span data-pw-sale-hms></span></span>':'';
    badges='<span class="pw-badge-sale pw-badge-sale-'+sale.kind+(sale.promoKind?' pw-badge-sale-'+sale.promoKind:'')+'">'+sale.badge+'</span>';
  } else if(badge){
    badges='<span class="pw-for-you-badge">'+COPY.forYou+'</span>';
  }
  ...
  if(bdayPct>0&&!(p&&p.isClearance===true)){
    ...
    if(bdayBadge)badges+='<span class="pw-badge-birthday">'+bdayBadge+'</span>';
  }
...
function renderRecommendedCard(p,badge){
  ...
  if(badge){
    var rec='<span class="pw-rec-badge">'+COPY.recBadge+'</span>';
    if(mark.indexOf('pw-card-promo-badges')>=0)mark=mark.replace('<div class="pw-card-promo-badges">','<div class="pw-card-promo-badges">'+rec);
    else mark='<div class="pw-card-promo-badges">'+rec+'</div>'+mark;
  }
...
function revealLiveProducts(el){
  el.setAttribute('data-pw-live-products','ready');
}
function loadFlashSale(el){
  var st=el._pwGrid;if(!st||st.loading)return;
  var cta=el.getAttribute('data-cta')||COPY.viewCta;
  var grid=el.querySelector('[data-pw-grid]');
  var empty=el.querySelector('.pw-personalize-empty');
  el._pwFlashGen=(el._pwFlashGen||0)+1;
  var gen=el._pwFlashGen;
  var tries=0;
  st.loading=true;
  function pull(){
    apiFetch('/flash-sale').then(function(res){done(res);}).catch(function(){done(null);});
  }
  function done(res){
    if(el._pwFlashGen!==gen)return;
    var failed=!res||res.ok===false||!!(res.j&&res.j.retry);
    if(failed){
      if(tries<2){
        tries++;
        setTimeout(pull,tries===1?600:1400);
        return;
      }
      st.loading=false;
      if(el._pwFlashPainted){
        revealLiveProducts(el);
        el.hidden=false;
        return;
      }
      el._pwFlashFails=(el._pwFlashFails||0)+1;
      if(el._pwFlashFails>4)return;
      setTimeout(function(){
        if(!el._pwGrid)return;
        el._pwGrid.loading=false;
        loadFlashSale(el);
      },4000);
      return;
    }
    ...
    if(!products.length){
      var painted=!!el._pwFlashPainted;
      el._pwFlashEmpty=(el._pwFlashEmpty||0)+1;
      if(painted&&el._pwFlashEmpty<2){
        st.loading=false;
        revealLiveProducts(el);
        el.hidden=false;
        setTimeout(function(){
      ...
      el._pwFlashPainted=false;
      st.hasMore=false;paintMore(el);revealLiveProducts(el);el.hidden=true;
...
function loadPersonalizePage(el,append){
  ...
  var st=el._pwGrid;if(!st||st.loading)return;
  if(kind==='flash-sale'){loadFlashSale(el);return;}
  var cta=el.getAttribute('data-cta')||COPY.viewCta;
  ...
  apiFetch(personalizePath(el,reqOffset,st.pageSize)).then(function(res){
    ...
    if(guestWithoutSignal&&!append){
      ...
      st.hasMore=false;paintMore(el);revealLiveProducts(el);el.hidden=false;return;
    }
    ...
    document.querySelectorAll('[data-pw-personalize]').forEach(function(el){
      ...
      if(el.getAttribute('data-pw-personalize')==='flash-sale'){
        ...
        if(flashGrid&&!el._pwFlashPainted){
          flashGrid.innerHTML='';
          el.setAttribute('data-pw-live-products','loading');
        }
        hydrateBlock(el);
        return;
      }
