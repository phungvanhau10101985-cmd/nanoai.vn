...
test('shop theme CSS keeps desktop account nav as a compact left column', () => {
  ...
  assert.match(css, /\.pw-shop-loyalty-hero\{/)
  assert.match(css, /\.pw-shop-loyalty-bar>span/)
  ...
  assert.match(css, /\.pw-shop-order-filters[^{]*\{[^}]*display:flex;flex-wrap:nowrap[^}]*overflow-x:auto/)
  assert.match(css, /\.pw-shop-account-hub-order-row\{[^}]*flex-wrap:nowrap[^}]*overflow-x:auto/)
  assert.match(css, /\.pw-shop-order-filter-chip[^{]*\{[^}]*flex:1 0 auto[^}]*white-space:nowrap/)
  assert.match(css, /\.pw-shop-order-filter-chip\.is-active[^{]*\{[^}]*border-color:var\(--pw-primary\);color:var\(--pw-primary\)/)
  assert.doesNotMatch(css, /\.pw-shop-order-filters[^{]*\{[^}]*display:grid/)
  assert.doesNotMatch(css, /@media\(min-width:768px\)\{\.pw-shop-order-filters\{display:grid/)
  assert.doesNotMatch(css, /@media\(min-width:768px\)\{\.pw-shop-order-filters\{grid-template-columns/)
  assert.match(css, /\.pw-shop-order-card\{[^}]*border-top:3px solid color-mix\(in srgb,var\(--pw-primary\)/)
  assert.match(css, /\.pw-shop-order-card-top\{[^}]*border-bottom:1px solid var\(--pw-border/)
  assert.match(css, /\.pw-shop-order-actions\{[^}]*border-top:1px solid var\(--pw-border/)
  assert.match(css, /@media\(max-width:767px\)\{[\s\S]*\.pw-shop-order-actions\{display:grid;grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/)
  ...
  assert.match(css, /@media\(min-width:900px\)\{\.pw-shop-deposit-body\{grid-template-columns:5fr 7fr\}/)
  assert.match(css, /\.pw-shop-deposit-level\{grid-column:1\/-1/)
  assert.match(css, /@media\(min-width:900px\)\{\.pw-shop-deposit-level\{padding:12px 16px\}\.pw-shop-deposit-opts\{flex-direction:row/)
  assert.match(css, /\.pw-shop-deposit-qr img\{width:min\(86vw,300px\)/)
  ...
  assert.match(css, /\.pw-shop-empty\{/)
  assert.match(css, /\.pw-shop-skel::after/)
  assert.match(css, /\.pw-shop-account-avatar\{/)
  assert.match(css, /\.pw-shop-wallet-card\{/)
  assert.match(css, /\.pw-shop-notif-card\{/)
})
...
test('shop theme CSS sizes stock bottom-nav icons from chrome vars', () => {
  ...
  assert.doesNotMatch(css, /\.pw-pdp-sticky-nav button\.is-fav,.pw-pdp-sticky-nav \[data-pw-chrome-btn="favorite-product"\]\{color:#fff\}/)
  assert.doesNotMatch(css, /button\.is-fav svg,.pw-pdp-sticky-nav \[data-pw-chrome-btn="favorite-product"\] svg\{color:#fff/)
...
test('shop theme CSS keeps listing filters compact and sticky under the head', () => {
  ...
  assert.match(css, /data-pw-listing-facet-modal/)
  assert.match(css, /data-pw-listing-facet-modal-close/)
...
test('shop theme CSS keeps mobile cart totals on a full-width row, not the 22px checkbox column', () => {
  ...
  assert.match(css, /@media\(min-width:1280px\)\{\.pw-shop-cart-layout\{/)
  assert.match(css, /@media \(min-width:1280px\)\{[\s\S]*\.pw-shop-cart-row\{grid-template-columns:24px 72px minmax\(0,1fr\) minmax\(7\.5rem,auto\)/)
...
test('shop theme CSS keeps cart summary money and buttons inside the viewport', () => {
  ...
  assert.match(css, /\.pw-shop-cart-kv-v\{[^}]*white-space:normal!important/)
  assert.match(css, /\.pw-shop-cart input:not\(\[type="checkbox"\]\):not\(\[type="radio"\]\),[\s\S]*min-width:0!important/)
  assert.match(css, /\.pw-shop-cart-stack,[\s\S]*flex-direction:column!important/)
  assert.match(css, /\.pw-shop-account-content:has\(\.pw-shop-cart\)\{[^}]*overflow-x:visible!important/)
  assert.match(css, /\.pw-shop-cart-promo-row\{display:flex;flex-direction:column/)
  assert.match(css, /html \.pw-newsletter[\s\S]*flex-direction:column!important/)
  assert.match(css, /html \.pw-newsletter input[\s\S]*min-width:0!important/)
  assert.doesNotMatch(css, /\.pw-shop-cart-summary\{[^}]*overflow-x:clip/)
  assert.doesNotMatch(css, /@media\(min-width:768px\)\{\.pw-shop-cart-actions\{flex-direction:row\}/)
  assert.match(css, /@media \(min-width:1280px\)\{[\s\S]*grid-template-columns:minmax\(0,1fr\) max-content/)
