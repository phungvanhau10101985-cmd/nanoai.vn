...
test('public storefront does not use a Suspense fallback that can consume the first click', async () => {
  ...
  assert.match(source, /PARTNER_SHOP_MOBILE_HEADER_LOGO_SCRIPT/)
  assert.match(source, /PARTNER_SHOP_MOBILE_HEADER_LOGO_SCRIPT_ID}-early/)
...
test('image search and compose pages boot without Suspense or nested startTransition', async () => {
  ...
  assert.doesNotMatch(imageClient, /useSyncExternalStore/)
