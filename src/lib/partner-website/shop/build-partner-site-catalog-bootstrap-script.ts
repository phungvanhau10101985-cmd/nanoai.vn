...
import { PW_LISTING_FACET_PICKER_CLOSE_SVG } from '@/lib/partner-website/shop/partner-site-listing-facet-picker'
...
import { PW_LISTING_BATCH_SIZE, PW_PRODUCT_GRID_PAGE_JS } from '@/lib/partner-website/shop/pw-product-grid-page'
import {
...
export function buildPartnerSiteCatalogBootstrapScript(input: {
  ...
  const copy = {
    ...
    filterClose: shopCopy.categoryFilterClose,
...
function dropListingSeeAll(root){
  var nodes=root.querySelectorAll('[data-pw-el="section-more"],.pw-grid-all');
...
function gridHasMore(j, offset){
  if(!j)return false;
  if(j.hasMore===true)return true;
  var total=Number(j.inventoryTotal!=null?j.inventoryTotal:(j.total!=null?j.total:NaN));
...
function paintMore(el){
  ...
  var st=el._pwGrid;
  if(categoryListingEl(el)){
    more.hidden=!!(st&&st.settled&&!st.hasMore);
    return more;
  }
...
function categoryListingEl(el){
  if(isRelated(el))return false;
  if(el.getAttribute('data-pw-personalize'))return false;
  if(el.getAttribute('data-pw-outfit')==='1'||el.getAttribute('data-pw-grid-kind')==='outfit')return false;
  if((el.getAttribute('data-category-id')||'').trim())return true;
  if((el.getAttribute('data-pw-listing-href')||'').trim())return true;
  var doc=document.documentElement;
  if(doc&&doc.getAttribute('data-pw-listing-category')==='1')return true;
  var body=document.body;
  if(body&&body.getAttribute('data-pw-listing-category')==='1')return true;
...
function queryFor(el,offset,limit){
  var page=Math.max(1,Math.min(48,limit||catalogPageSize(el)));
...
function listingFacetPickerSelects(bar){
  if(!bar)return [];
  return Array.prototype.filter.call(bar.querySelectorAll('select[data-pw-facet],select[data-pw-el="sort"]'),function(sel){
    var k=sel.getAttribute('data-pw-facet');
    return k!=='min_price'&&k!=='max_price';
  });
}
function syncListingFacetTrigger(sel){
  if(!sel)return;
  var wrap=sel.closest('[data-pw-listing-facet-wrap]');
  var btn=wrap&&wrap.querySelector('[data-pw-listing-facet-trigger]');
  if(!btn)return;
  var opt=sel.options[sel.selectedIndex];
  btn.textContent=opt?String(opt.textContent||''):'';
}
var listingFacetPickerSel=null;
function closeListingFacetPicker(){
  var host=document.querySelector('[data-pw-listing-facet-modal]');
  if(host){
    host.hidden=true;
    host.setAttribute('aria-hidden','true');
  }
  document.documentElement.removeAttribute('data-pw-listing-facet-open');
  var sel=listingFacetPickerSel;
  listingFacetPickerSel=null;
  if(sel){
    var wrap=sel.closest('[data-pw-listing-facet-wrap]');
    var btn=wrap&&wrap.querySelector('[data-pw-listing-facet-trigger]');
...
function ensureListingFacetPicker(){
  var host=document.querySelector('[data-pw-listing-facet-modal]');
  if(host)return host;
  host=document.createElement('div');
  host.setAttribute('data-pw-listing-facet-modal','1');
  host.setAttribute('hidden','');
  host.setAttribute('aria-hidden','true');
  host.setAttribute('role','dialog');
  host.setAttribute('aria-modal','true');
  host.innerHTML='<button type="button" data-pw-listing-facet-modal-backdrop="1"></button>'
    +'<div data-pw-listing-facet-modal-card="1">'
    +'<div data-pw-listing-facet-modal-head="1">'
    +'<h3 data-pw-listing-facet-modal-title="1"></h3>'
    +'<button type="button" data-pw-listing-facet-modal-close="1" aria-label="">'+${JSON.stringify(PW_LISTING_FACET_PICKER_CLOSE_SVG)}+'</button>'
    +'</div>'
    +'<div data-pw-listing-facet-modal-list="1" role="listbox"></div>'
    +'</div>';
  document.body.appendChild(host);
  host.addEventListener('click',function(ev){
    var t=ev.target;if(!t||!t.closest)return;
    if(t.closest('[data-pw-listing-facet-modal-close],[data-pw-listing-facet-modal-backdrop]')){
      ev.preventDefault();
      closeListingFacetPicker();
      return;
    }
    var opt=t.closest('[data-pw-listing-facet-option]');
    if(!opt||!listingFacetPickerSel)return;
    ev.preventDefault();
    var v=opt.getAttribute('data-pw-listing-facet-option')||'';
    var sel=listingFacetPickerSel;
  ...
  document.addEventListener('keydown',function(ev){
    if(ev.key==='Escape'&&document.documentElement.getAttribute('data-pw-listing-facet-open')){
      ev.preventDefault();
      closeListingFacetPicker();
    }
  });
...
function openListingFacetPicker(sel){
  if(pwShopLiveUiOff()||!sel)return;
  listingFacetPickerSel=sel;
  var host=ensureListingFacetPicker();
  var title=host.querySelector('[data-pw-listing-facet-modal-title]');
  var list=host.querySelector('[data-pw-listing-facet-modal-list]');
  var closeBtn=host.querySelector('[data-pw-listing-facet-modal-close]');
  var label=sel.getAttribute('aria-label')||'';
  if(title)title.textContent=label;
  if(closeBtn){
    closeBtn.setAttribute('aria-label',COPY.filterClose||'');
  }
  var backdrop=host.querySelector('[data-pw-listing-facet-modal-backdrop]');
  if(backdrop)backdrop.setAttribute('aria-label',COPY.filterClose||'');
  var html='';
  Array.prototype.forEach.call(sel.options||[],function(opt){
    var v=String(opt.value||'');
    var selected=String(sel.value||'')===v;
    html+='<button type="button" class="pw-listing-facet-option'+(selected?' is-selected':'')+'" data-pw-listing-facet-option="'+esc(v)+'" role="option" aria-selected="'+(selected?'true':'false')+'">'+esc(opt.textContent||v)+'</button>';
  });
  if(list){
    list.innerHTML=html;
...
function enhanceListingFacetSelect(sel){
  if(!sel||sel.closest('[data-pw-listing-facet-wrap]')){
    syncListingFacetTrigger(sel);
    return;
  }
  var parent=sel.parentNode;if(!parent)return;
  var wrap=document.createElement('span');
  wrap.setAttribute('data-pw-listing-facet-wrap','1');
  wrap.className='pw-listing-facet-wrap';
  ...
  btn.type='button';
  btn.className='pw-listing-facet-trigger';
  btn.setAttribute('data-pw-listing-facet-trigger','1');
  ...
  btn.addEventListener('click',function(ev){
    ...
    openListingFacetPicker(sel);
  });
}
function enhanceListingFacetSelects(bar){
  listingFacetPickerSelects(bar).forEach(enhanceListingFacetSelect);
}
...
function syncListingControlsFromUrl(){
  ...
  listingFacetPickerSelects(bar).forEach(syncListingFacetTrigger);
  var clear=ensureListingClearBtn(bar);
  clear.hidden=!listingFacetOn(p)&&!p.sort;
}
function releaseListingGate(){
  document.documentElement.setAttribute('data-pw-listing-filters-ready','1');
}
function holdListingPending(el,on){
  if(!el||!listingCatalogEl(el))return;
  if(on)el.setAttribute('data-pw-listing-pending','1');
  else el.removeAttribute('data-pw-listing-pending');
}
function listingGenCurrent(el){
  return el&&el._pwGridGen||0;
}
function paintListingFacets(j){
  if(pwShopLiveUiOff())return;
  var bar=listingFilterBar();
  if(!bar){ releaseListingGate(); return; }
  ...
  enhanceListingFacetSelects(bar);
...
function bindListingFilters(){
  ...
  if(!bar){
    var root=document.querySelector('[data-pw-inline-visual-root]');
    var page=document.documentElement.getAttribute('data-pw-page')||'';
    var listing=page==='listing'||(root&&root.getAttribute('data-pw-listing-category')==='1');
    if(listing)releaseListingGate();
    return;
  }
  if(bar.getAttribute('data-pw-listing-filters-bound')==='1')return;
  bar.setAttribute('data-pw-listing-filters-bound','1');
...
function finishListingPaint(el,j,append){
  if(append||!listingCatalogEl(el))return;
  paintListingFacets(j);
  releaseListingGate();
...
function loadGridPage(el,append){
  ...
  fetchJsonOnce(API+queryFor(el,st.offset,st.pageSize)).then(function(res){
    ...
    if(!res.ok){
      ...
      st.settled=true;st.hasMore=categoryListingEl(el)&&st.offset>=st.pageSize;paintMore(el);revealLiveCatalog(el);el.hidden=false;
    ...
    if(!products.length){
      ...
      st.settled=true;st.hasMore=false;paintMore(el);revealLiveCatalog(el);el.hidden=false;
      finishListingPaint(el,res.j,append);
      return;
    }
...
function refreshSeededCatalog(el){
  ...
  fetchJsonOnce(API+queryFor(el,0,st.pageSize)).then(function(res){
    if(listingGenCurrent(el)!==gen)return;
    st.loading=false;
    var products=(res.j&&res.j.products)||[];
    if(isRelated(el)){
      var exclude=currentProductId(el);
    ...
    if(!res.ok){
      st.settled=true;st.hasMore=categoryListingEl(el)&&st.offset>=st.pageSize;paintMore(el);
      holdListingPending(el,false);
      releaseListingGate();
      return;
    }
    if(after===before){
      st.settled=true;st.hasMore=gridHasMore(res.j,st.offset);
      paintMore(el);
      finishListingPaint(el,res.j,false);
      return;
    }
    st.offset=0;
    st.hasMore=true;
...
function hydrate(el){
  el._pwGridGen=(el._pwGridGen||0)+1;
  el._pwGrid={offset:0,pageSize:catalogPageSize(el),hasMore:true,loading:false,settled:false};
  holdListingPending(el,true);
  loadGridPage(el,false);
}
...
function run(){
  ...
  if(!document.documentElement.getAttribute('data-pw-listing-gate-timer')){
    document.documentElement.setAttribute('data-pw-listing-gate-timer','1');
