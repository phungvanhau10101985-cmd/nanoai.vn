...
export const DEMO_PDP_BIND_PRODUCT: LivePdpBindProduct = {
  ...
  depositPolicy: true,
  stockQty: 4,
  brandName: SOURCE.category?.parent?.name || '188 Fashion',
  origin: 'Trung Quốc',
  material: SOURCE.material || 'Voan',
  style: 'Hàn Quốc',
  occasion: 'Dạo phố',
  likesCount: 128,
  purchasesCount: 86,
  ...
  questionsCount: 6,
  productInfo: {
    product_info: { sku: 'DEMO-PDP-001', name: SOURCE.name, brand: '188 Fashion', origin: 'Trung Quốc' },
    specifications: { upper_material: SOURCE.material || 'Voan', style: 'Hàn Quốc', occasion: 'Dạo phố' },
    variants: { colors: SOURCE.colors.map((c) => c.name), sizes: ['S', 'M', 'L', 'XL'] },
  },
