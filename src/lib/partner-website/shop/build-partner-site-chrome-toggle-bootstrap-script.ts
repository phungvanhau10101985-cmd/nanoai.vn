...
function ensureCatBackdrop(){
  ...
  if(!el){
    ...
    el.addEventListener('pointerdown',function(){el.__pwCatBackdropPress=1;});
    el.addEventListener('click',function(e){
      var press=el.__pwCatBackdropPress;
      el.__pwCatBackdropPress=0;
      if(press){
...
function scheduleCatLeave(btn,panel){
  clearCatLeave(btn);
  if(!btn)return;
  if(btn.__pwCatSuppressLeaveUntil&&Date.now()<btn.__pwCatSuppressLeaveUntil)return;
  btn.__pwCatLeaveT=setTimeout(function(){closeEl(btn,panel||ensureCatPanel(btn));},150);
}
function suppressCatLeave(btn){
  if(!btn)return;
  clearCatLeave(btn);
  btn.__pwCatSuppressLeaveUntil=Date.now()+500;
}
function relatedInCatUi(related,btn,panel){
  if(!related)return false;
  if(panel&&(related===panel||(panel.contains&&panel.contains(related))))return true;
  if(btn&&(related===btn||(btn.contains&&btn.contains(related))))return true;
  if(related.closest&&related.closest('[data-pw-cat-acc-backdrop],.pw-cat-acc-backdrop'))return true;
...
function bindCatHover(btn){
  ...
  function openOnHover(){
    ...
    if(livePanel&&!catPanelReady(livePanel)){
      stripSeedCatLinks(livePanel);
      hydrateCats(false);
      return;
    }
    var root=deviceRoot(btn);
...
function bindToggles(){
  ...
  for(i=0;i<catBtns.length;i++){
    ...
    btn.addEventListener('click',function(e){
      ...
      var liveAccBtn=qs(root,accBtnSel());
      var liveAcc=liveAccBtn?qs(root,accPanelSel()):qs(root,accPanelSel());
      requestCatOpen(cur,livePanel,liveAccBtn,liveAcc);
    });
  }
  ...
  if(!document.documentElement.getAttribute('data-pw-chrome-toggle-doc')){
    ...
    window.addEventListener('pointerdown',function(e){
      var ptr=String((e&&e.pointerType)||'');
      if(ptr==='touch'||ptr==='pen')return;
    ...
    document.addEventListener('click',function(e){
      ...
      if(t.closest('[data-pw-cat-acc-backdrop]'))return;
      if(catSwallowClick(e))return;
      if(catIgnoreClick()){
        e.preventDefault();
        e.stopPropagation();
      ...
      if(hit&&!isInsidePanel(hit,panelSel())&&!isInsidePanel(hit,accPanelSel())){
        ...
        var liveAccBtn=qs(root,accBtnSel());
        var liveAcc=liveAccBtn?qs(root,accPanelSel()):qs(root,accPanelSel());
        requestCatOpen(hit,livePanel,liveAccBtn,liveAcc);
        return;
      }
      if(t.closest(panelSel())||t.closest('.pw-chrome-cat-wrap'))return;
      if(anyCatGestureFresh())return;
...
function catPanelReady(panel){
  return !!(panel&&panel.getAttribute('data-pw-cat-filled')==='1');
}
function stripSeedCatLinks(panel){
  if(!panel||catPanelReady(panel))return;
  var kids=panel.children;
  var i;
  for(i=kids.length-1;i>=0;i--){
    var n=kids[i];
    if(n&&n.tagName&&String(n.tagName).toLowerCase()==='a'){
      try{n.remove();}catch(errSeed){}
    }
  }
}
function stripAllSeedCatLinks(){
  var panels=document.querySelectorAll(panelSel());
  var i;
  for(i=0;i<panels.length;i++)stripSeedCatLinks(panels[i]);
}
var pendingCatOpenBtn=null;
function flushPendingCatOpen(){
  var btn=pendingCatOpenBtn;
  ...
  var liveAccBtn=qs(root,accBtnSel());
  var liveAcc=liveAccBtn?qs(root,accPanelSel()):qs(root,accPanelSel());
  toggleCatPair(btn,panel,liveAccBtn,liveAcc);
}
function requestCatOpen(btn,panel,otherBtn,otherPanel){
  if(!btn||!panel)return;
  if(!catPanelReady(panel)){
  ...
  pendingCatOpenBtn=null;
  if(hoverCapable()&&!isMobileCatFace()&&panel.classList.contains('is-open'))return;
  toggleCatPair(btn,panel,otherBtn,otherPanel);
}
var hydrateCatsInFlight=false;
var hydrateCatsCoolUntil=0;
function hydrateCats(force){
  if(pwShopLiveUiOff())return;
  if(hydrateCatsInFlight)return;
  if(!force&&Date.now()<hydrateCatsCoolUntil)return;
...
function catGestureFresh(btn){
  ...
  return !!(at&&Date.now()-at<800);
}
function anyCatGestureFresh(){
  var nodes=document.querySelectorAll(catSel());
  var i;
  for(i=0;i<nodes.length;i++){
    if(catGestureFresh(nodes[i]))return true;
  }
  return false;
}
function catIgnoreClick(){
  return !!(window.__pwCatIgnoreClickUntil&&Date.now()<window.__pwCatIgnoreClickUntil);
}
...
if(!window.__pwChromeToggleMo){
  window.__pwChromeToggleMo=1;
  var moTimer=null;
  var mo=typeof MutationObserver!=='undefined'?new MutationObserver(function(recs){
    ...
    moTimer=setTimeout(function(){
      ...
      var need=false,i;
      for(i=0;i<panels.length;i++){
        if(!panels[i].getAttribute('data-pw-cat-filled')){need=true;break;}
      }
      if(need)hydrateCats(false);
    },100);
  }):null;
