...
test('fashion home seed is stamped and matches live visual contract', () => {
  ...
  assert.match(html, /<section[^>]*data-pw-personalize="flash-sale"/)
  assert.match(html, /data-pw-grid-kind="flash-sale"[^>]*data-limit="12"|data-limit="12"[^>]*data-pw-grid-kind="flash-sale"/)
  ...
  assert.match(html, /data-pw-chrome-size="22"/)
...
test('seedShopTemplateVisualWebsite writes every built-in page for all four devices', () => {
  ...
  assert.ok((seeded.theme.visualPageKeys ?? []).includes('how_to_buy'))
  assert.ok((seeded.theme.visualPageKeys ?? []).includes('brand_origin'))
  assert.ok((seeded.theme.visualPageKeys ?? []).includes('reviews_policy'))
  assert.ok((seeded.theme.visualPageKeys ?? []).includes('trust'))
  assert.ok((seeded.theme.visualPageKeys ?? []).includes('company'))
  ...
  assert.match(about.content, /data-pw-chrome-size="22"/)
})
...
test('fashion-marketplace seed stamps look and home API hooks on all four devices', () => {
  ...
  for (const variant of VISUAL_DEVICE_VARIANTS) {
    ...
    assert.match(home.content, /<section[^>]*data-pw-personalize="flash-sale"/)
