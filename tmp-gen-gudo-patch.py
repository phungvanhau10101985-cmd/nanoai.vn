...
copy = {
  ...
  "contact": "Liên hệ qua Chat mua trên website. Địa chỉ trụ sở sẽ cập nhật sau khi hoàn tất đăng ký hộ kinh doanh.",
  "email": "hotro@gudo.vn",
...
import json
js_copy = json.dumps(copy, ensure_ascii=False)
js_company = json.dumps(company, ensure_ascii=False)
js_contact = json.dumps(contact, ensure_ascii=False)
src = r'''const { readFileSync, existsSync } = require('fs')
...
'''
src = src.replace('__COPY__', js_copy).replace('__COMPANY__', js_company).replace('__CONTACT__', js_contact)
Path(r'e:\python-code\Thu-do-online\tmp-patch-gudo-footer.cjs').write_text(src, encoding='utf-8')
print('wrote', len(src))