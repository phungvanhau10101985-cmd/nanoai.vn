# Kiểm tra QR template (`payment_configs`)

## Cách 1: Giao diện bảng (Table editor) — nhanh

### Bước 1: Mở database
1. Kết nối tới Postgres của bạn (host cung cấp thường có **Table editor** / **Data** trong dashboard).
2. Chọn **đúng** database / schema `public`.

### Bước 2: Vào danh sách bảng
```
Menu → Table editor (hoặc tương đương)
```
...
## Cách 2: SQL Editor / `psql` (chính xác)

### Bước 1: Mở SQL client
```
Menu → SQL Editor (hoặc psql nối DATABASE_URL)
```
...
**Nên bắt đầu với Cách 1 (Table editor) để xem nhanh cột `qr_template_url`.**